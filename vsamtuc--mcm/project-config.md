---
trigger: always_on
description: - `cmd/mcm/main.go` boots `internal/app.App` for dependency init, then exposes `http.Server` on `:8080` using `internal/transport/http.NewMux` (which now needs both a schema readiness callback and a `course.Store`).
---
...
---
> Source: [vsamtuc/mcm](https://github.com/vsamtuc/mcm) — distributed by [TomeVault](https://tomevault.io).
