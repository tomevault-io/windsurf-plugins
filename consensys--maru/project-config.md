---
trigger: always_on
description: Maru is an Ethereum Consensus client that implements QBFT Consensus algorithm. It's meant to be used on Ethereum L2-s,
---
...
## Validation (Java 25 / Gradle)
Use **JDK 25** locally so behavior matches GitHub Actions (`actions/setup-java` uses the same version).

- Run a full compile and test pass: `./gradlew build` (or `make build`).
- If formatting or generated output drifts, run `./gradlew spotlessApply` or `make spotless-happy`, then re-check with
  `./gradlew spotlessCheck`.
- After changing the runtime image or Java baseline, confirm the Docker base tag resolves, for example:
  `docker build -f app/Dockerfile …` per [`README.md`](README.md) (see “Build Docker Image Locally”).

## Changelog
For every PR, update `CHANGELOG.md` under the appropriate section:
- `### Upcoming Breaking Changes` — for upcoming breaking changes
...
- Before considering the work complete, ALWAYS make sure:
  - The whole project compiles on **JDK 25** (`./gradlew build` succeeds with `java -version` reporting 25)
  - The code is properly formatted

---
> Source: [Consensys/maru](https://github.com/Consensys/maru) — distributed by [TomeVault](https://tomevault.io).
