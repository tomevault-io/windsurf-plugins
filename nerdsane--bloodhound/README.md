# bloodhound

> Source: [nerdsane/bloodhound](https://github.com/nerdsane/bloodhound) — distributed by [TomeVault](https://tomevault.io)

Deterministic simulation testing platform for hunting bugs in distributed systems

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [nerdsane/bloodhound](https://github.com/nerdsane/bloodhound).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [nerdsane/bloodhound](https://github.com/nerdsane/bloodhound) — a repo with 20+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
