---
trigger: always_on
description: Follow this workflow for every Go-related task:
---
...
```bash
python3 .claude/skills/goskill/scripts/search.py "<project description>" --arch-system -p "ProjectName"
```
...
python3 .claude/skills/goskill/scripts/search.py "functional options config" --domain pattern

# Error handling approach
python3 .claude/skills/goskill/scripts/search.py "error wrapping context" --domain error

# Concurrency patterns
python3 .claude/skills/goskill/scripts/search.py "worker pool bounded" --domain concurrency

# Performance tips
python3 .claude/skills/goskill/scripts/search.py "http connection pooling" --domain perf

# Testing patterns
python3 .claude/skills/goskill/scripts/search.py "table driven httptest" --domain test
```

### 4. Stack Guidelines
If using a specific framework/stack:
```bash
python3 .claude/skills/goskill/scripts/search.py "middleware authentication" --stack web-gin
python3 .claude/skills/goskill/scripts/search.py "cobra subcommands config" --stack cli
python3 .claude/skills/goskill/scripts/search.py "interceptor logging" --stack grpc
```
...
---
> Source: [vurakit/agentup](https://github.com/vurakit/agentup) — distributed by [TomeVault](https://tomevault.io).
