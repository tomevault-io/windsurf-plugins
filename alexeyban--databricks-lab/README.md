# databricks-lab

> Source: [alexeyban/databricks-lab](https://github.com/alexeyban/databricks-lab) — distributed by [TomeVault](https://tomevault.io)

A Databricks and PySpark learning laboratory, fully architected and developed by AI agents.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [alexeyban/databricks-lab](https://github.com/alexeyban/databricks-lab).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [alexeyban/databricks-lab](https://github.com/alexeyban/databricks-lab) — a repo with 20+ stars on GitHub.

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-s -->
