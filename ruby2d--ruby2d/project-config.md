---
trigger: always_on
description: Run `rake` to test: it reinstalls the gem (rebuilding the native C extension) and runs the full RSpec suite in `spec/`. Never use `gem build`, `gem install`, or `rspec` directly.
---
