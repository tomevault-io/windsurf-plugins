---
trigger: always_on
description: 本项目为现代化多语言、响应式的案例展示与用户互动平台，基于 Next.js 14 App Router，TypeScript，Tailwind CSS，Shadcn UI，Framer Motion，Supabase（认证+数据库），next-intl（国际化）等技术栈实现。下述规则为后续所有页面（如用户中心）开发的技术和结构标准。
---
