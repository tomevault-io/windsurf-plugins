---
trigger: always_on
description: Reccoo is a Windows-only, single-window WPF application targeting `net9.0-windows`. The project intentionally uses a flat structure:
---
...
Reccoo is a Windows-only, single-window WPF application targeting `net9.0-windows`. The project intentionally uses a flat structure:
...
`dotnet run` launches a debug build. The Release build matches CI and treats warnings as errors. The publish command creates the distributable `publish/Reccoo.exe`.
...
---
> Source: [darkdarkcocoa/Reccoo](https://github.com/darkdarkcocoa/Reccoo) — distributed by [TomeVault](https://tomevault.io).
