---
trigger: always_on
description: GitHub Issues，外部 PR 也作为 triage 需求来源。见 `docs/agents/issue-tracker.md`。
---
...
### 权威状态与数据面 / Authoritative State & Data Plane
- **单一权威**：跨端共享的状态（图数据、频道/频段定义、冲突标志）只在持有权威的一侧计算（本仓 = **服务端**）。客户端可以补足它**本地能证明**的部分，但**绝不下调它证明不了的部分**。
  （曾出现：客户端把冲突标志重算成「仅同图重名」，抹掉了服务端送来的跨方块冲突。）
- **最小数据面**：权威值变化时只发**与该变化等量**的数据（节点级 op / 针对性数据包），**不要**用整图 NBT 推送代替 —— 那会冲掉正在进行的编辑（`isGraphReplaceBlocked(pendingLocalOps)` 这类守卫正是为它存在的）。
  （曾出现：为了通知一个频段列表的变化而走整图推送，与正在编辑的玩家相互覆盖。）
- **不本端重算权威值**：本端推导（读缓存 / 注册表 / 启发式）只允许作为**一次性初始化**或**可证明的补充**，不得成为每 tick / 每次收包后的常规路径。
  （曾出现：客户端每帧按本地频段表刷新 BUS_IN 频段 —— 各端各自推导导致分叉，并覆盖服务端刚下发的权威值。）
- **权威值必须被持续维护**：不能假设「解析一次就永远正确」。定义变化后**所有**持有该值的实体都要被更新到（含其它方块 / 其它宿主）；**自愈式不变量优于一次性事件推送**。
  （曾出现：只刷新「发起变更的那个方块」，其它方块里的旧值永远过期，且重开界面也无效 —— 因为权威侧自己的副本就是旧的。）
- **命名对齐要检查两端**：把 A 的值对齐给同名的 B 时，**A 与 B 都要过资格检查**（是否持有权威、是否冲突 / 失效）。
  （曾出现：只检查来源、没检查目标，把频道持有者的频段图写进了冲突节点，毁掉它自己的图。）
...
---
> Source: [y15173334444/create-schematic-compute](https://github.com/y15173334444/create-schematic-compute) — distributed by [TomeVault](https://tomevault.io).
