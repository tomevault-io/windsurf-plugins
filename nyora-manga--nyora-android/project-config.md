...
# AI Coding Agent Instructions (Nyora / Kotatsu fork)
...
- External sources: Manga parsers via `com.github.AppNyora:nyora-parsers:$parsersVersion` (JitPack).
...
- Tests:
  ...
  - Instrumented: `./gradlew connectedAndroidTest` (runner `com.nyora.android.HiltTestRunner`).
...
- Package layout under `io/github/landwarderer/nyora/`:
...
- Uses JitPack `nyora-parsers` with version from `libs.versions.toml`.
- Override for testing:
  - `./gradlew assembleDebug -DparsersVersionOverride=<short-sha>`
  - Example: `curl -s https://api.github.com/repos/appnyora/nyora-parsers/commits/master -H "Accept: application/vnd.github.sha" | cut -c -10`.
...
- Manifest defines many deep links (`DetailsActivity`, `ReaderActivity`, etc.) and app-specific schemes (`nyora://…`); validate flows when changing intents.
...
---
> Source: [Nyora-Manga/nyora-android](https://github.com/Nyora-Manga/nyora-android) — distributed by [TomeVault](https://tomevault.io).
