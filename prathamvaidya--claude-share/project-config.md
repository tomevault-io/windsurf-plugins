---
trigger: always_on
description: Single npm package (`@0xpv/claude-share`) exposing two binaries: `claude-share` (sharer) and `claude-connect` (receiver). Source lives in `claude-share/src/` and `claude-connect/src/` respectively; both compile into their own `dist/` via the root `package.json`.
---
