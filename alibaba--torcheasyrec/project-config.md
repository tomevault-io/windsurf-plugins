---
trigger: always_on
description: Canonical instructions for AI coding agents in this repository. `CLAUDE.md` imports this file; keep shared agent guidance here.
---
...
- Before running project commands, inspect the repository's declared PyTorch version and use an existing project conda env or venv with a matching PyTorch version. If none exists or a required tool is missing, stop and ask; do NOT install substitutes or new toolchains on your own.
...
- **Inline `#` comments are minimized**: code should be self-explanatory; comment only non-obvious "why" context, one short line. No commented-out code, no change-history or tombstone comments. Use `# TODO(username): ...` and `# NOTE: ...`; do not introduce FIXME/XXX/HACK. Denser math commentary is fine in `tzrec/ops/` kernels. Don't create trivial 1-2 line single-use helpers unless they significantly improve readability.
- Code, comments, and docstrings are English-only; prefer plain ASCII in new comments outside `tzrec/ops/`.
- Keep user-facing docs in `docs/` and write them in Chinese, focusing on configuration, operation, observable behavior, and troubleshooting. Place internal artifact layouts and implementation contracts beside the code that enforces them.
...
- Assume the reader knows PyTorch and TorchRec. Match existing patterns; if uncertain, choose the simpler, more concise implementation. Make the smallest surgical change that solves the request: reuse existing helpers, data structures, and control flow; add no speculative features, abstractions, configurability, fallbacks, or defenses for impossible states; do not refactor or reformat adjacent code; remove only orphans introduced by your change.

---
> Source: [alibaba/TorchEasyRec](https://github.com/alibaba/TorchEasyRec) — distributed by [TomeVault](https://tomevault.io).
