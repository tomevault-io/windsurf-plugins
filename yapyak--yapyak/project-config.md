---
trigger: always_on
description: `yapyak` is a Vite-first i18n library. The runtime translation function is `t()`. The core package publishes **unscoped** as `yapyak`. Framework adapters, translator providers, and the Vite plugin publish as **public** scoped packages under `@yapyak/*` (e.g. `@yapyak/react`, `@yapyak/vite`, `@yapyak/anthropic`). Internal-only workspace packages are also `@yapyak/*` but marked `private` — `@yapyak/typescript-config`, `@yapyak/tsdown-config`, `@yapyak/vitest-config`, `@yapyak/docs-compiler`, `@yap
---
...
- `packages/{anthropic,claude-code,gemini,ollama,openai}/` — LLM translator providers (`@yapyak/<provider>`)
...
- **Process:** `yapyak-workflow` · `yapyak-style` · `yapyak-commit`

---
> Source: [yapyak/yapyak](https://github.com/yapyak/yapyak) — distributed by [TomeVault](https://tomevault.io).
