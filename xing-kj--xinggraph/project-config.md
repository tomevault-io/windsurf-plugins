---
trigger: always_on
description: This document summarizes how to work with the xinggraph repository: how it’s organized, how to build, test, lint, and contribute. It mirrors our actual tooling and CI while providing quick commands for local development.
---

## Repository Guidelines

This document summarizes how to work with the xinggraph repository: how it’s organized, how to build, test, lint, and contribute. It mirrors our actual tooling and CI while providing quick commands for local development.

## Project Structure & Module Organization

- `xinggraph/`: Core Python library and API.
  ...
  - `cli/`: CLI entry points and subcommands invoked via `xinggraph` / `xinggraph-cli`.
...
- `xinggraph-mcp/`: Model Context Protocol server exposing xinggraph as MCP tools (SSE/HTTP/stdio). Contains its own README and Dockerfile.
- `xinggraph-frontend/`: Next.js UI for local development and demos.
...
uv run xinggraph-cli -ui   # Launches UI, backend API, and MCP server together
```

- Start the FastAPI server directly:
```bash
uv run python -m xinggraph.api.client
```
...
- Prefer explicit, structured error handling. Use shared logging utilities in `xinggraph.shared.logging_utils`.

MCP server and Frontend:
- Follow the local `README.md` and ESLint/TypeScript configuration in `xinggraph-frontend/`.

## Testing Guidelines

- Place Python tests under `xinggraph/tests/`.
  - Unit tests: `xinggraph/tests/unit/`
  - Integration tests: `xinggraph/tests/integration/`
  - CLI tests: `xinggraph/tests/cli_tests/`
...
---
> Source: [xing-kj/XingGraph](https://github.com/xing-kj/XingGraph) — distributed by [TomeVault](https://tomevault.io).
