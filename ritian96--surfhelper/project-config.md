---
trigger: always_on
description: - **操作系统**: Windows 64位 (win32)
---
...
| **JavDB影片管理器** | 2.1.0 | `https://javdb.com/*` | 影片自动屏蔽与智能评分；搜索精确高亮、详情页状态同步、悬浮大图预览、数据管理与备份、一键批量爬取；支持日产/欧美/FC2番号 |
...
#### 🛡️ 内容管理类
- **JavDB影片管理器**: 看过/想看影片屏蔽，智能评分分级（低分/高分）；批量导入，状态同步，大图预览，数据备份；支持日产/欧美/FC2番号

#### 🔗 链接处理类
- **微博磁链补全助手**: 自动识别并补全磁力链接（支持 weibo.com 及子域名）
...
2. **功能验证** - 用户在实际环境测试功能。若有多个子任务，需全部完成后再进入下一步。
3. **验收与文档（统一执行）** - **当所有功能开发并验证通过后，由 CLI 主动提议更新版本号和文档。** 经用户确认后，一次性更新脚本版本号、`.md` 文档、README.md 等。
   - 禁止在开发过程中的中间步骤碎片化地更新文档和版本号。
4. **发布** - 确认文档无误后，推送到 GitHub。
...
└── tampermonkey-scripts/       # 油猴脚本目录
    ...
    ├── javdb-manager.user.js # JavDB管理器脚本 (v2.1.0)
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/RiTian96)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/RiTian96)
