...
# **Project Ellen: Core Architecture & AI Implementation**

This section details the specific implementation of the "Ellen" project, a desktop companion app driven by Generative AI.

## **AI Core (`GeminiCore`)**

The brain of the application is the `GeminiCore` class (`lib/gemini_core.dart`). It wraps the `firebase_ai` package to provide a persistent persona.

### **System Instruction & Persona**
The AI is initialized with a specific personality prompt:
*   **Name**: Ellen.
*   **Personality**: Discretely friendly, slightly lazy/sarcastic, candy lover, efficient.
*   **Language**: Spanish (always).
*   **Context Injection**:
    *   **Date**: Current date/time is injected into the system prompt.
    *   **Expressions**: The top 10 most used emotional expressions are injected to influence her mood.

### **Expressions System**
Ellen expresses emotions via text tags formatted as `[expression]`.
*   **Syntax**: `[feliz]`, `[enojada]`, `[sorprendida]`, `[idle]`, etc.
*   **Handling**:
    *   `ExpressionManager` parses these tags from the AI response.
    *   It updates the usage count in Firestore (`expressions` collection).
    *   It triggers `AnimationLinkManager` to switch the background video loop to match the emotion.
    *   `TtsService` strips these tags so they are not spoken aloud.

### **Command System**
The AI can perform actions using a custom command syntax in its text response: `[comando"nombre_comando" arg1="val" arg2="val"]`.

**Supported Commands:**
1.  **Create Event**: `[comando"crear_evento" titulo="..." fecha="..." categoria="..." recordatorio="..."]`
    *   Parses arguments and uses `EventManager` to save to Firestore.
2.  **Create Task**: `[comando"crear_tarea" descripcion="..." categoria="..." prioridad="..." etiquetas="tag1,tag2"]`
    *   Creates a `Task` object and saves via `TaskManager`.
3.  **Check Agenda**: `[comando"consultar_agenda" fecha="YYYY-MM-DD"]`
    *   Queries `EventManager` and `TaskManager` for that date and returns a summary to the chat context.

## **Core Services**

### **Text-to-Speech (`TtsService`)**
*   **Provider**: Google Cloud TTS API (`https://texttospeech.googleapis.com/v1/text:synthesize`).
*   **Voice**: `es-US-Chirp3-HD-Leda` (Spanish US, Female).
*   **Format**: `LINEAR16` (WAV) for high fidelity and smooth playback.
*   **Logic**:
    1.  Splits input text by sentence and expression tags.
    2.  Fetches audio for text chunks in parallel.
    3.  Plays audio chunks sequentially using `AudioPlayerService`.
    4.  Triggers animations (via `onExpression` callback) in sync with the flow of speech.

### **Speech-to-Text (`SttService`)**
*   **Provider**: `speech_to_text` package (local device recognition).
*   **Locale**: `es_ES`.
*   **Modes**:
    *   **Standard**: Listens for a single utterance/command.
    *   **Live Chat**: Continuous listening mode (toggleable) for fluid conversation.

### **State & Data Management**
*   **Firestore**: Primary database.
    *   `events`: Calendar events.
    *   `tasks`: To-do list items (synced with Google Tasks if authenticated).
    *   `expressions`: Usage statistics for Ellen's emotional state.
    *   `chat_history`: Persistent conversation logs.
*   **Local Config**: `AppConfig` manages API keys and local settings (stored in `shared_preferences` conceptually, loaded from file/env).

## **UI Structure**
*   **`HomeScreen`**: The main container.
    *   **Background**: `VideoManager` plays the current `_currentPlaylist` (looped video).
    *   **Sidebar**: `SidebarWidget` for history access, settings, and navigation.
    *   **Chat**: `ChatWidget` handles the message list, STT input, and typing indicators.
    *   **Overlays**: Note container (minimized/maximized) and Event details bottom sheet.
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/niguerrac)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/niguerrac)
