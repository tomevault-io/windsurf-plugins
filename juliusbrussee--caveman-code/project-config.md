...
tmux new-session -d -s cave-test -x 80 -y 24

# Start cave from source
tmux send-keys -t cave-test "cd $(pwd) && ./test.sh" Enter

# Wait for startup, then capture output
sleep 3 && tmux capture-pane -t cave-test -p

# Send input
tmux send-keys -t cave-test "your prompt here" Enter

# Send special keys
tmux send-keys -t cave-test Escape
tmux send-keys -t cave-test C-o  # ctrl+o

# Cleanup
tmux kill-session -t cave-test
```
