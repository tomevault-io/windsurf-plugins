---
trigger: always_on
description: - 使用 Gradle Wrapper：`./gradlew`，Windows 下等价命令是 `.\gradlew.bat`。
---
...
- 新的数据能力按这条路径走：先改 `feature-library-data` 中的 `storage/api` 接口，再补 `storage/impl` 实现与 Koin 绑定，最后在 domain facade 和 `AppModule` 注入到对应 ViewModel。
...
---
> Source: [yangSpica27/SPICaMusic_Android](https://github.com/yangSpica27/SPICaMusic_Android) — distributed by [TomeVault](https://tomevault.io).
