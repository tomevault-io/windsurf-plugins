# Gloss

> Source: [RecursiveIntell/Gloss](https://github.com/RecursiveIntell/Gloss) — distributed by [TomeVault](https://tomevault.io)

A local-first, privacy-preserving alternative to Google NotebookLM — Tauri 2, React 19, Rust, Ollama

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [RecursiveIntell/Gloss](https://github.com/RecursiveIntell/Gloss).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [RecursiveIntell/Gloss](https://github.com/RecursiveIntell/Gloss) — a repo with 38+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
