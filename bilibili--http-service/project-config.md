...
- `pnpm test` - Run Jest tests (from `/packages/http-svc/`)
...
- Tests run automatically before build (`prebuild` hook)
...
- **Pattern**: `**/*.spec.ts`
- **Config**: `packages/http-svc/jest.config.ts`
- Run tests with `pnpm test` in the http-svc package
- Run specific test: `pnpm test -- --testNamePattern="test name"`

## Code Quality

- **ESLint**: TypeScript configuration with Prettier integration (`.eslintrc.js`)
- **Line Length**: 140 characters maximum  
- **Formatting**: Prettier for consistent code style
- No explicit lint/format scripts - integrated into development workflow

---
> Source: [bilibili/http-service](https://github.com/bilibili/http-service) — distributed by [TomeVault](https://tomevault.io).
