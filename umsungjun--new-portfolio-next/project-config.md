...
- **i18n:** next-intl — 한국어(기본) 및 영어, `localePrefix: "as-needed"` 설정으로 기본 언어(한국어)는 URL prefix 없음, 영어만 `/en` prefix 포함
...
모든 페이지는 `app/[locale]/` 하위에 위치. URL 구조:
- `/` — 한국어 홈 (기본 locale, prefix 없음)
- `/en` — 영어 홈

미들웨어(`middleware.ts`)가 구형 경로를 301 영구 리다이렉트로 처리:
- `/home`, `/ko/home` → `/`
- `/en/home` → `/en`
- trailing slash 정규화 (예: `/en/` → `/en`)
- 유효하지 않은 경로는 모두 `/`로 리다이렉트
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/umsungjun) — claim your Tome and manage your conversions.
