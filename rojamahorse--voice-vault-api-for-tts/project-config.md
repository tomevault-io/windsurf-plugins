---
trigger: always_on
description: ﻿# Development Guide for Pinokio Projects
---
...
2. **Example Lock-in:** Identify the closest matching script in `F:\pinokio\prototype\system\examples`. Record its path and keep it open while editing. Every launcher change must mirror that reference unless the user explicitly instructs otherwise.
3. **Pre-flight Checklist:** Convert the applicable rules from this document and `PINOKIO.md` at F:\pinokio\prototype\PINOKIO.md into a task-specific checklist (install/start/reset/update structure, regex patterns, menu defaults, log checks, etc.). Confirm each item is ticked **before** making changes.
...
- Make sure to keep this entire document and `PINOKIO.md` at F:\pinokio\prototype\PINOKIO.md in memory with high priority before making any decision. Pinokio is a system that makes it easy to write launchers through scripting by providing various cross-platform APIs, so whenever possible you should prioritize using Pinokio API over lower level APIs.
- When writing pinokio scripts, ALWAYS check the examples folder (in F:\pinokio\prototype\system\examples folder) to see if there are existing example scripts you can imitate, instead of assuming syntax.
- When implementing pinokio script APIs and you cannot infer the syntax just based on the examples, always search the API documentation `PINOKIO.md` at F:\pinokio\prototype\PINOKIO.md to use the correct syntax instead of assuming the syntax.
...
IMPORTANT: ALWAYS try to follow the best practices in the examples folder (F:\pinokio\prototype\system\examples) instead of trying to come up with your own structure. The examples have been optimized for the best user experience.
...
- If starting from scratch, first determine what type of project you will be building, and then check the examples folder (F:\pinokio\prototype\system\examples) to see if you can adopt them instead of coming up everything from scratch.
...
1. Check the examples in the F:\pinokio\prototype\system\examples folder
2. Read the `PINOKIO.md` at F:\pinokio\prototype\PINOKIO.md further documentation on the full syntax
...
- The `pinokio.js` launcher script can change dynamically depending on the current state of the script execution. Which means, depending on what the file returns, it can determine what the sidebar looks like at any given moment of the script cycle.
  ...
  - Check the examples in the F:\pinokio\prototype\system\examples folder to see how these are being used.
...
- **Pinokio Programming:** See `PINOKIO.md` at F:\pinokio\prototype\PINOKIO.md â†’ "Programming Pinokio" section
- **Dynamic Menus:** See `PINOKIO.md` at F:\pinokio\prototype\PINOKIO.md â†’ "Dynamic menu rendering" section  
- **CLI Commands:** See `PTERM.md` at F:\pinokio\prototype\PTERM.md
### Common Patterns
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/Rojamahorse)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/Rojamahorse)
