# neo

> Source: [Parslee-ai/neo](https://github.com/Parslee-ai/neo) — distributed by [TomeVault](https://tomevault.io)

A self-improving code reasoning engine with persistent semantic memory

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [Parslee-ai/neo](https://github.com/Parslee-ai/neo).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [Parslee-ai/neo](https://github.com/Parslee-ai/neo) — a repo with 12+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
