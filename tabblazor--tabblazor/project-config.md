---
trigger: always_on
description: TabBlazor is a Blazor component library built on the [Tabler](https://preview.tabler.io/) CSS framework (Bootstrap-based admin dashboard UI). It provides 90+ Razor components with minimal JS interop. NuGet packages: `TabBlazor` and `TabBlazor.QuickTable.EntityFramework`.
---
...
TabBlazor is a Blazor component library built on the [Tabler](https://preview.tabler.io/) CSS framework (Bootstrap-based admin dashboard UI). It provides 90+ Razor components with minimal JS interop. NuGet packages: `TabBlazor` and `TabBlazor.QuickTable.EntityFramework`.
...
Minimal JS in `src/TabBlazor/wwwroot/js/tabblazor.js` (~200 lines). All interop goes through `TablerService`. Handles: theme switching, clipboard, file downloads, scrolling, element manipulation. Avoid adding new JS interop unless strictly necessary.
...
- **NEVER use Conventional Commits prefixes** (`feat:`, `fix:`, `chore:`, `refactor:`, etc.). The summary must start directly with an imperative verb.
...
---
> Source: [TabBlazor/TabBlazor](https://github.com/TabBlazor/TabBlazor) — distributed by [TomeVault](https://tomevault.io).
