# dotfiles

> Source: [joelazar/dotfiles](https://github.com/joelazar/dotfiles) — distributed by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [joelazar/dotfiles](https://github.com/joelazar/dotfiles).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

## Bundled Skills (9)

- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/uv/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/reddit/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/github/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/exe-dev/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/mermaid/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/simplify/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/web-search/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/listen-later/SKILL.md)
- [dotfiles](https://github.com/joelazar/dotfiles/tree/main/dot_agents/skills/session-analyzer/SKILL.md)

From [joelazar/dotfiles](https://github.com/joelazar/dotfiles) — a repo with 166+ stars on GitHub.

---

Install this config instantly:
```
npx tomevault install joelazar/dotfiles
```
Source: [github.com/joelazar/dotfiles](https://github.com/joelazar/dotfiles).

<!-- genome:d-i-s -->
