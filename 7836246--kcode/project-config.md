---
trigger: always_on
description: - 新增或修改行为前，先更新对应 spec；目录不存在时按需创建。先明确产品规则、状态所有者、接口和验收场景，再实现代码。
---
...
- 组件通过 `packages/ui/src/hooks/` 访问服务；平台操作通过 `IPlatformService`（`packages/shared/src/platform.ts`），不直接调用 `window.kcode`。
...
- Desktop app 通过 stdio 与 Agent 通信。协议改动同步更新 `packages/shared/src/kcode-protocol/index.ts`，提供严格类型与运行时校验。
...
- UI 使用 `packages/ui/src/logger.ts`，不直接使用 `console.log` 或 `window.kcode?.log`。
...
---
> Source: [7836246/kcode](https://github.com/7836246/kcode) — distributed by [TomeVault](https://tomevault.io).
