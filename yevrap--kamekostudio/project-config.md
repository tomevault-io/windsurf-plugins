---
trigger: always_on
description: Kameko Studio is a one-person web game studio. All games are mobile-first progressive web apps built with vanilla HTML, CSS, and JavaScript — no frameworks, no backend, no build step. Everything is static files deployed on GitHub Pages at https://yevrap.github.io/KamekoStudio/.
---
...
- **Tap to Spell:** Mirrors T9 Smart — `autoFillScrollSpecials()` appends consecutive specials after each confirmed char; answer auto-submits via shared `autoSubmitTimer`.
- **`autoSubmitTimer`:** Shared 300ms `setTimeout` for both T9 Smart and Tap to Spell auto-submit. **Cancelled by back key, ok key, and `showNextPrompt()` — NOT cancelled on mode switch** (closure captures the answer at schedule time; fires correctly regardless of current mode).
- **Mode switching preserves input progress:** `setInputMode()` converts typed progress across modes — `t9buf.length` (scroll) / `t9pos` (predict) / `ki.value` prefix match (keyboard). Pending scroll char (`t9pend`) is always cleared. `t9timer` (scroll auto-confirm) is cancelled on mode switch.
- **Keyboard mode:** Uses a near-invisible fixed `<input>` (1×1px, opacity 0.01) at viewport center so focusing it never causes scroll/layout shift on mobile. Submit handlers capture `ki.value` then clear it **before** calling `submitAnswer()` so display always sees an empty field. `showNextPrompt()` also clears `ki.value` in keyboard mode to handle all advance paths.
...
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/yevrap)
