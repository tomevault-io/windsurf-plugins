---
trigger: always_on
description: SelectedTextKit 是一个 Swift Package，通过 Accessibility、AppleScript、菜单操作和
---
...
详细规则位于 `docs/agents/`；公开的 Package 使用文档位于根目录 `README.md`。
...
- PR 交付：遵循 `.agents/skills/submit-pr/SKILL.md` 和
  `docs/agents/git-workflow.md`；按目标 checkout 发现实际仓库、分支和 remote。
...
- 语言规则：除非用户明确要求其他语言，否则使用简体中文沟通；仓库治理文档、计划、
  历史、参考资料、仓库维护的 skill 和 Agent 配置使用中文，代码注释使用英文。
  直接镜像的上游 skill 保留上游语言；代码标识、API 名称、命令、路径和品牌名称保留原文。
...
---
> Source: [tisfeng/SelectedTextKit](https://github.com/tisfeng/SelectedTextKit) — distributed by [TomeVault](https://tomevault.io).
