---
trigger: always_on
description: - When the user specifically asks to generate a markdown file, open it after writing. Always open markdown files using `,markdown-to-html <filepath>`. For HTML or other file formats, use `open <filepath>` with `dangerouslyDisableSandbox: true` — the sandbox blocks `open` from launching GUI apps.
---
