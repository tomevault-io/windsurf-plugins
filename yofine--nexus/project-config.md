---
trigger: always_on
description: Nexus 是一个**本地 Web 控制台**，用于在单个浏览器界面中同时管理多个 CLI AI Agent 实例（Claude Code、OpenCode 等）的并行协作。用户通过 `nexus` CLI 启动服务，自动打开浏览器进入管理界面。
---
...
---
> Source: [yofine/Nexus](https://github.com/yofine/Nexus) — distributed by [TomeVault](https://tomevault.io).
