...
- **只要“监控与查看”（状态/统计/模型/日志/配置读取）**：可以用容器，但你必须显式提供：
  ...
  - 面板访问密钥：`CLIPROXY_PANEL_PANEL_ACCESS_KEY`，至少 32 字符；缺失时拒绝启动
...
- 面板自身
  ...
  - `CLIPROXY_PANEL_BIND_HOST`：本机直接运行默认 `127.0.0.1`；只有明确需要时才改为非回环地址
  - `CLIPROXY_PANEL_PANEL_ACCESS_KEY`：本机监听时可选；非回环监听时必须至少 32 字符。设置后 `/api/*` 只接受 `X-Panel-Key`。浏览器首次设置可用 `#panel_key=...` fragment 写入本地存储；禁止用会进入服务器与代理日志的查询参数
...
- 默认值优先安全可部署（监听默认 `127.0.0.1`、主配置写回默认关闭；非回环监听必须配强访问密钥）
- 自动探测必须“只补缺省值”，不能破坏用户显式配置
- 不要恢复前端导出入口；主配置写回默认保持关闭

---
> Source: [ferretgeek/cliproxyapi-dashboard](https://github.com/ferretgeek/cliproxyapi-dashboard) — distributed by [TomeVault](https://tomevault.io).
