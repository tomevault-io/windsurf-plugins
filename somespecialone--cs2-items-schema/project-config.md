---
trigger: always_on
description: This repository generates and stores an understandable, deliberately simple & normalized Counter-Strike 2 item schema from upstream game files. Consumers use the checked-in JSON resources in `schemas/` or relational DDL/data in `sql/`. The dataset comes only from game files and is intentionally not guaranteed to contain every item.
---
...
---
> Source: [somespecialone/cs2-items-schema](https://github.com/somespecialone/cs2-items-schema) — distributed by [TomeVault](https://tomevault.io).
