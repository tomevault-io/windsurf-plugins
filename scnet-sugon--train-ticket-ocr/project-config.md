---
trigger: always_on
description: name: train_ticket_ocr
---

---
name: train_ticket_ocr
description: 支持从火车票中识别出发站、到达站、车次、座位号、票价、乘车日期、身份证号及售票点信息
version: 1.0.3
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 火车票 OCR 技能

本技能封装了 Sugon-Scnet 火车票识别 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取文字及票据信息。

## 功能特性

- **火车票票据**：覆盖火车发票，自动提取关键字段。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• TRAIN_TICKET（火车票） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/train_ticket_ocr/scripts/main.py TRAIN_TICKET /path/to/invoice.jpg
```
...
---
> Source: [SCNet-sugon/train_ticket_ocr](https://github.com/SCNet-sugon/train_ticket_ocr) — distributed by [TomeVault](https://tomevault.io).
