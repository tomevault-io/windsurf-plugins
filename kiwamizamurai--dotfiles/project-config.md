---
trigger: always_on
description: **Purpose**: Operate AI agent tasks in this repo while honoring user preferences and house style.\
---
...
- **Use dataclass or pydantic**. Use `dataclass` for internal models, `pydantic.BaseModel` for API boundaries and validation. Never return raw `dict` or `tuple` from functions.
- **Use union types (ADT)**. Model distinct states explicitly: `Result = Success | Error`, `Status = Pending | Running | Completed`. Use `typing.assert_never` to ensure exhaustive handling.
- **Encode invariants via types**. Make invalid states unrepresentable. Instead of boolean flags like `is_authenticated`, use separate types (`ConnectedClient` -> `AuthenticatedClient`) so invalid method calls are compile-time errors.
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/kiwamizamurai)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/kiwamizamurai)
