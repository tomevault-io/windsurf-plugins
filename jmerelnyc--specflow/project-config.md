...
3. **读取 `.specflow/v{N}/05_TASKS.md`** → 了解当前待办
4. **开始工作**
...
| `.specflow/` | **统一架构根目录**。包含版本化架构状态与升级记录。 | **只读**(旧版) / **写一次**(新版) / `changelog` 由 CLI 维护。 |
| `.specflow/v{N}/` | **当前真理**。最新的架构定义。 | 永远寻找最大的 `v{N}`。 |
| `.specflow/changelog/` | **升级记录**。`specflow update` 生成的变更记录。 | 由 CLI 自动维护，请勿删除。 |
...
| `/probe` | 变更前 / 接手项目 | `.specflow/v{N}/00_PROBE_REPORT.md` |
...
| `/upgrade` | `specflow update` 后做升级编排 | 判断 Minor / Major，并路由到 `/change` 或 `/genesis` |
...
- **最新架构版本**: `.specflow/v7`
- **活动任务清单**: `.specflow/v7/05_TASKS.md`
- **待办任务数**: 11
- **最近一次更新**: `2026-04-01`
...
- 完成标志: `AGENTS.md` / `README` / `CLI` / `update` 语义与 `.specflow/v7` 保持一致，且 v7 里程碑完成
...
- **在新架构就绪前**: 请勿大规模修改代码。
- **架构总览**: `.specflow/v7/02_ARCHITECTURE_OVERVIEW.md`
- **ADR**: `.specflow/v7/03_ADR/` (跨系统决策的唯一记录源)
- **遇到架构问题**: 请查阅 `.specflow/v7/03_ADR/`。
...
- install-state-registry: 维护 `.specflow/install-lock.json` 与状态漂移修复
...
- 下一执行入口: 继续按 `.specflow/v7/05_TASKS.md` 收口剩余任务与里程碑验证
...
---
> Source: [jmerelnyc/specflow](https://github.com/jmerelnyc/specflow) — distributed by [TomeVault](https://tomevault.io).
