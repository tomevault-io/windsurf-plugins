---
trigger: always_on
description: 20年前の PowerPC Mac（iBook）を、現代のネットワーク環境で実用できるようにする。
---
...
**作業方針:**
- ファイルを変更する前に `.bak` を取る。20年前のハードで、壊すと復旧が難しい
...
**iBook 上には現代的な開発ツールチェーンが無い**（PPC 向け Node.js が存在しない等）。
...
**翻訳の文言は最終的に自分で決める。**
自動生成された候補をそのまま採用せず、自然さは自分の判断で確定させる。
...
**要件: 自己完結。** 別筐体（ブリッジサーバ）を常設運用に組み込むことをゴールとしない。
iBook 単体（+ 母艦での一時的なビルド作業）で完結し、インストールして単体で動くアプリを
最終形とする。放っておくと楽な方に流れやすいので、ここは明記しておく。
...
**Samba 4 を PPC で自力ビルドしようとしないこと。**

- ~~ビルドシステム（waf）が Python 3.6+ を要求し、PPC への Python 3 移植が独立した大プロジェクトになる~~
  → **誤りだった。** MacRumorsスレッドで barracuda156氏（2026-08-22）から訂正あり。
  PPCPorts（PPC専用のPorts集、MacPortsとほぼ並行してリリース）に Samba4 のビルド済み最新版が既に存在する。
  Python3移植が壁になるという前提は古い情報で、現在は成立しない。
- **ただし方針変更の結論は変わらない。** 仮にビルド済み/自力ビルドで Samba4 が使えても、
  得られるのは `smbclient`（CLI のみ）で、**Finder にボリュームとしてマウントされるわけではない。**
  タスク2のゴールは「Finder にアイコンが出てD&Dできること」であり、ビルド可否は最初から論点ではなかった。
  libsmb2 + WebDAVループバック方式を選んだ理由はここにある。
...
#### A案: ブリッジサーバ（あくまで保険。ゴールではない）

**格下げ済み。** 自己完結の要件と矛盾するため、常設の解決策としては採用しない。
C案が詰まった場合の一時しのぎ、または動作確認用の比較対象としてのみ使う。
放置すると「ここで満足して終わる」方向に流れやすいので、既定の進行順（後述）には含めない。
...
---
> Source: [watermark-hd/ppc-mac-modernization](https://github.com/watermark-hd/ppc-mac-modernization) — distributed by [TomeVault](https://tomevault.io).
