---
trigger: always_on
description: Bus Pirate 5/6/7 firmware — embedded C firmware for RP2040/RP2350 microcontrollers. Universal serial interface supporting 1-Wire, I2C, SPI, UART, JTAG, infrared, and other protocols. Dual-core architecture: Core 0 handles UI/commands, Core 1 handles LCD/display.
---
...
When adding translations, put the key-value pair in `en-us.h`, then run `src/translation/json2h.py` to apply them to the whole system. If it isn't possible to run the script, do not touch `base.h` and provide a list of `[key]="value"` pairs to include in a format ready to manually paste in `en-us.h`.
...
---
> Source: [DangerousPrototypes/BusPirate5-firmware](https://github.com/DangerousPrototypes/BusPirate5-firmware) — distributed by [TomeVault](https://tomevault.io).
