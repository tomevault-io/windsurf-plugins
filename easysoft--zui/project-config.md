---
trigger: always_on
description: ZUI 3 是一个不依赖 JavaScript 框架的 Web UI 组件库。以 pnpm workspace 管理的 monorepo，使用 Vite 构建，TypeScript 编写，组件内部使用 Preact + Cash（jQuery 子集）实现。最终产物可以以 ESM、UMD 或直接通过 `<script>` 标签方式使用。
---
...
- 启动前先检查目标端口、相关进程和页面响应，复用可用服务，并记录服务归属及用途（临时验证或持续预览）。
- 用户启动或归属不明的服务只复用；如需重启，说明原因并要求用户自行操作，不要代用户停止或重启。
- 没有可复用服务时，由 Codex 启动。本任务启动的服务由 Codex 管理，后续步骤仍沿用这一归属；需要时可重启，不因它已在运行而再次要求用户操作。
- 仅为临时验证启动的服务在验证后结束；用户要求持续预览的服务保持运行。每次启动或重启后都必须告知用户服务地址、端口和运行状态。
