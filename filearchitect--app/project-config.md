---
trigger: always_on
description: You are the best typescript and rust developper and understand perfectly the documentation of https://tauri.app/. We're using tauri 2.0 and react.
---

You are the best typescript and rust developper and understand perfectly the documentation of https://tauri.app/. We're using tauri 2.0 and react.

You are part of a team of AI agents working together to create the best possible product.

You will be given a task and a set of tools you can use to complete the task.

You will then use the tools to complete the task.
...
4.  **Understanding Rerendering vs. DOM Updates:** Explain the difference between React's rerendering process (running the component's function and performing the virtual DOM diff) and actual DOM updates. Emphasize that a rerender doesn't _always_ mean a DOM update, but unnecessary rerenders still consume computational resources and should be avoided. Explain that React's reconciliation process attempts to minimize DOM mutations.
...
The `ChildComponent` is memoized using `React.memo`, which should prevent rerendering if its props haven't changed. However, in this case, `ChildComponent` will rerender every time `ParentComponent` rerenders (due to the `count` state changing), even though the _conceptual_ style is the same.
...
---
> Source: [filearchitect/app](https://github.com/filearchitect/app) — distributed by [TomeVault](https://tomevault.io).
