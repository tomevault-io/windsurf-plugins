---
trigger: always_on
description: 本文档用于约束 AI 在本仓库中的协作方式，帮助 AI 快速理解项目结构、查找项目文档，并将每次协作过程中的知识沉淀到知识库。
---
...
- `Docs/Knowledge`：知识库，AI 每次协作后沉淀的知识总结目录。
  - `Docs/Knowledge/开发约定与模式.md`：提炼文档——项目编码约定、测试模式、跨平台策略
  - `Docs/Knowledge/领域与技术参考.md`：提炼文档——布局管线、高亮架构、UTF-16 等
  - `Docs/Knowledge/任务日志/`：按日期组织的任务总结（`YYYY-MM-DD-任务简述.md`）
  - `Docs/Knowledge/通用参考/`：偏通用技术知识，不绑定具体任务
  - `Docs/Knowledge/_archive/`：已过时但保留备查
  - `Docs/Knowledge/README.md`：完整索引与使用说明
...
1. 确保更改部分都有考虑单元测试
   - 单元测试有多个项目，需将各个单元测试项目都加入考虑
   - 单元测试应该全面覆盖，不能只考虑某几个方法
   - 任务完成后，运行单元测试项目，确保单元测试通过
     - 单元测试应该使用 `dotnet test -c Release` 方式运行，必须加上 `-c Release` 参数
2. 每次 AI 完成任务后，必须将本次协作知识总结写入 `Docs/Knowledge/任务日志/` 目录。

要求：

- 每次任务产出一份独立文档。
- 文件名：`YYYY-MM-DD-任务简述.md`。
- 内容至少包括：
  - 背景与目标
  - 关键文件与模块
  - 主要决策与原因
  ...
  - 验证方式（构建/测试/手工验证）
  - 后续建议
- 若是纯通用技术知识（不绑定具体任务），放入 `Docs/Knowledge/通用参考/`
- 完成后更新 `Docs/Knowledge/README.md` 索引表

3. 如果发现知识库文档较多，则总结经验，更新本文档

---
> Source: [dotnet-campus/LightTextEditorPlus](https://github.com/dotnet-campus/LightTextEditorPlus) — distributed by [TomeVault](https://tomevault.io).
