...
- CLI in `qsiprep/cli/` (parser.py, run.py, workflow.py, version.py)
- Singleton config in [config.py](mdc:qsiprep/config.py) with sections: environment, execution, workflow, nipype, seeds
- Workflows in `qsiprep/workflows/` using Nipype's `pe.Workflow`, `pe.Node`, `pe.MapNode`
- Interfaces in `qsiprep/interfaces/` following Nipype patterns
- Tests in `qsiprep/tests/` with pytest; integration tests gated by markers
...
---
> Source: [PennLINC/qsiprep](https://github.com/PennLINC/qsiprep) — distributed by [TomeVault](https://tomevault.io).
