# template

> Source: [snowdreamtech/template](https://github.com/snowdreamtech/template) — distributed by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `.cursor/rules/*.mdc` in [snowdreamtech/template](https://github.com/snowdreamtech/template).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [snowdreamtech/template](https://github.com/snowdreamtech/template) — a repo with 0+ stars on GitHub.

---

Install this config instantly:
```
npx tomevault install snowdreamtech/template
```
Source: [github.com/snowdreamtech/template](https://github.com/snowdreamtech/template).

<!-- genome:d-i-s -->
