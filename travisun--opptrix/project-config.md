...
- After modifying code files, run `GRAPHIFY_VIZ_NODE_LIMIT=18000 graphify update .` to keep the graph current (AST-only, no API cost)
- HTML viz node cap for this repo is **18000** (default graphify limit is 5000). Always prefix graphify commands that regenerate `graph.html` with `GRAPHIFY_VIZ_NODE_LIMIT=18000`; for `graphify export html` also pass `--node-limit 18000`
