---
trigger: always_on
description: This file provides guidance to AI tooling when working with code in this repository.
---
...
Flags accepted by `bench-lab.sh`: `-f` bench config (mandatory), `-c` config-sets dir, `-i` nanobsd images dir, `-n` iterations (min 3, default 5), `-p` pkt-gen configs dir, `-d` results dir (default `/tmp/benchs`), `-r` report email. Extra opt-ins via env: `PMC=true` to collect hwpmc, `STATS=true` for extra stats, `BEFORE_CMD` / `DURING_CMD` / `AFTER_CMD` (all run on the DUT) to run a command respectively before traffic, during the blast (launched detached and self-triggered when traffic appears on `DURING_IF`, killed after `DURING_SAMPLE`s, output scp'd back — note a live per-second sample only yields data when the DUT is not CPU-saturated by the blast; at line rate the userland sampler starves and dumps one whole-blast lump, so use a `BEFORE_CMD`/`AFTER_CMD` counter delta for saturating benches — see the `bench-lab.sh` header comment), and after the sender exits, `SENDER_START_CMD_2` / `SENDER_ADMIN_2` when a single generator can't produce the load (see the header comment in `bench-lab.sh` for the 3-node and 4-node topologies).
...
---
> Source: [ocochard/netbenches](https://github.com/ocochard/netbenches) — distributed by [TomeVault](https://tomevault.io).
