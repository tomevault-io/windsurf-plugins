---
trigger: always_on
description: Root-scope development guide for `komari-theme-transit`.
---
...
Screenshot baselines are split by environment. CI compares against the committed
`tests/visual/snapshots/`, generated by `visual-baseline.yml` on a pinned
`macos-15` runner. Locally the config points at `tests/visual/snapshots-local/`,
which is gitignored: the first run writes the missing baselines and reports that
run as failed, and every run after that is a real comparison. Do not commit local
baselines, and do not run `test:visual:baseline` to "fix" a local mismatch — that
rewrites the authoritative set with this machine's rendering.
...
---
> Source: [yyy622hhh/komari-theme-transit](https://github.com/yyy622hhh/komari-theme-transit) — distributed by [TomeVault](https://tomevault.io).
