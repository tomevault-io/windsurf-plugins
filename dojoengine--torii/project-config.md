---
trigger: always_on
description: - Code lives in `crates/` with co-located tests under `src/tests` or `#[cfg(test)]`; tooling scripts and runtime assets (Katana DBs, spawn bundles, binaries) sit in `scripts/`, `types-test-db*/`, `spawn-and-move-db*/`, and `bin/`.
---
