---
trigger: always_on
description: `seed-create-component` 스킬이 구현 전에 참조하는 **canonical reference 집합**이다. 컴포넌트별 예시 모음이 아니라, 여러 레이어에서 재사용되는 원칙과 판단 기준을 모아 둔 체크리스트 역할을 한다.
---

# skills/seed-create-component/references

## 디렉토리 개요

`seed-create-component` 스킬이 구현 전에 참조하는 **canonical reference 집합**이다. 컴포넌트별 예시 모음이 아니라, 여러 레이어에서 재사용되는 원칙과 판단 기준을 모아 둔 체크리스트 역할을 한다.
