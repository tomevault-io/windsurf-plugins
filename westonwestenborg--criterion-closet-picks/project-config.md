---
trigger: always_on
description: This repository is an Astro static site for Criterion Closet Picks. Site code lives in `src/`: pages in `src/pages`, UI in `src/components`, layouts in `src/layouts`, data helpers in `src/lib`, and global styling in `src/styles/global.css`. Fonts and imported assets are in `src/assets`; static files such as `CNAME` and `tmdb-logo.svg` are in `public/`.
---
...
---
> Source: [westonwestenborg/criterion-closet-picks](https://github.com/westonwestenborg/criterion-closet-picks) — distributed by [TomeVault](https://tomevault.io).
