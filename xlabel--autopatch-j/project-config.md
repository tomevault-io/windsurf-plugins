---
trigger: always_on
description: AutoPatch-J 是一个面向 Java 代码修复的 Python CLI Agent。源码位于 `src/autopatch_j/`。
---
...
- `pytest -q`：运行确定性的完整回归测试；即使已配置真实 LLM 参数，也不会运行真实 LLM 集成测试。
- `pytest tests/test_source_read_tools.py -q`：运行单个聚焦测试文件。
- `pytest -q --run-live-llm`：在普通测试全部通过后运行真实 LLM 集成测试；仅在用户明确同意本次运行时使用。
...
- 默认验证顺序是聚焦测试后运行 `pytest -q`。真实 LLM 环境参数仅表示配置可用，不表示可以运行真实 LLM 集成测试。
- 只有用户明确要求或明确同意“运行真实 LLM 集成测试”后，才可执行 `pytest -q --run-live-llm`。不得从环境参数存在、任务要求“完整测试”或一般测试授权中推断该同意。
- 验证报告必须明确区分确定性完整回归与真实 LLM 集成测试，并在后者未运行时直接注明“未运行”。
...
- 结构性问题优先使用 CodeGraph：定义、调用关系、影响范围、架构流向和符号上下文。
- 查找字面量、日志文本、配置文本或文档内容时使用 `rg` / 文件读取。
- 如果 CodeGraph 未初始化，可以询问是否初始化；不要为了同一个结构问题反复 grep。

## Plan mode 沟通与计划铁律

- 本节仅适用于当前协作模式为 Plan mode 的任务。
- 对非简单需求，生成正式 `<proposed_plan>` 前，必须先完成事实定位，并用简明中文单独讲清核心方案：问题具体怎么解决、关键行为如何变化、用户最终会看到什么，以及为什么推荐该方案。
- 核心方案尚未得到用户明确确认时，不得生成详细执行计划；OpenSpec 流程、文件清单、测试矩阵、验证命令和防御措施不得替代或淹没核心方案。
- 用户已经在当前请求或前序对话中明确给出并确认核心方案时，无需重复确认，可以直接生成详细执行计划。
- 用户明确要求“先定位”或“先讨论”时，必须停留在对应阶段，不得提前推进到策略选择或正式 plan。
- 正式 `<proposed_plan>` 应被视为交给一个上下文已清空的全新 Codex CLI session 的完整任务说明。新会话可以读取当前仓库和该计划，但看不到形成计划前的任何对话，包括折叠的 commentary、口头共识和临时结论。
- 凡前序对话中已经确认、且会影响实施或验收的内容——包括核心方案、关键事实、行为取舍、用户可见结果、范围边界和重要约束——都必须汇总并重述进正式计划；不得使用“按前面讨论”“沿用上述方案”等依赖旧上下文的表述。
- 计划的结构、标题、顺序和篇幅应按任务本身组织，不强制套用固定模板，也不要求把检查项逐项写成独立章节。计划必须覆盖本任务中所有会影响实施判断的信息；目标与成功标准、改前/改后行为、范围与非目标、关键代码落位或数据流、边界与失败处理、公共接口或规格变化、兼容性取舍、测试与验收以及适用的 OpenSpec/验证流程，仅作为语义完整性检查项，不适用的内容无需机械展开。
- “详细”是指计划包含足以直接实施和验收的决策与信息，不是堆砌无关背景、历史过程或通用流程；核心方案必须位于计划前部并保持醒目。
- 如果新会话仍需依赖已经清空的对话，或自行猜测产品行为、技术方案、范围边界、关键取舍或验收标准，说明计划尚未完成，必须在当前规划会话补齐后再输出。正式计划完成后，只有执行时发现仓库状态变化、计划与当前事实冲突，或出现真实外部阻塞，才需要重新询问用户。
...
---
> Source: [xLabel/AutoPatch-J](https://github.com/xLabel/AutoPatch-J) — distributed by [TomeVault](https://tomevault.io).
