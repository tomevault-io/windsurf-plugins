---
trigger: always_on
description: `lib/` contains the app code. Keep network access in `lib/api/`, domain and state objects in `lib/models/`, screens and UI flows in `lib/pages/`, and reusable helpers in `lib/utils/`. The entry point is `lib/main.dart`.
---
