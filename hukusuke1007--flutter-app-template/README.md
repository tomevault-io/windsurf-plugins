# flutter_app_template

> Tome by [hukusuke1007](https://github.com/hukusuke1007/flutter_app_template), graded and kept true across every major AI platform by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [hukusuke1007/flutter_app_template](https://github.com/hukusuke1007/flutter_app_template).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

## Bundled Skills (1)

- [flutter_app_template](https://github.com/hukusuke1007/flutter_app_template/tree/main/.agents/skills/claude-md-manager/SKILL.md)

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/hukusuke1007/flutter_app_template](https://github.com/hukusuke1007/flutter_app_template)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:t-c-p -->
