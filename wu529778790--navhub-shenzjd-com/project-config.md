---
trigger: always_on
description: Compact guide for AI agents working in this repo. For full architecture details, see `CLAUDE.md`.
---
...
- **Pages are ISR (since 2026-08-24)**: `page.tsx` and `/c/[id]` use `export const revalidate = 21600` (6 h, aligned with data cache TTL). Report state is per-anon client-fetched from `/api/sites/dead-report-state` (no SSR cookie reads), so page HTML has no per-user dependency and can be cached / CDN-served. Restart the container to rebuild pages after an import; in-app navigation is zero-request (local state + `history.pushState`, see `HomeClient`).
...
- **Tailwind v4 — no `tailwind.config.js`**. Theme is CSS-only via `@theme inline` in `src/app/globals.css`. Use CSS custom properties (`var(--foreground)`, `var(--accent-500)`, …), not Tailwind color classes. **Light + dark themes**: `<html data-theme>` drives overrides — applied by an inline script in `src/app/layout.tsx` before hydration (localStorage `theme` wins, defaults to system `prefers-color-scheme`), toggled by `src/components/ThemeToggle.tsx`. Never hardcode colors; dark values come from `[data-theme="dark"]` in globals.css.
...
---
> Source: [wu529778790/navhub.shenzjd.com](https://github.com/wu529778790/navhub.shenzjd.com) — distributed by [TomeVault](https://tomevault.io).
