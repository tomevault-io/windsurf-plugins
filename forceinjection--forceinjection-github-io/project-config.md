...
- **禁止**使用系统自带的 Ruby（macOS 默认 2.6），否则 `bundle install` 和 `jekyll serve` 会报 Bundler 版本不匹配错误。
...
| `bundler: command not found: jekyll` | 未在项目 bundle 环境中执行                 | 先 `rbenv exec bundle install`，再 `rbenv exec bundle exec jekyll serve` |
...
---
> Source: [ForceInjection/forceinjection.github.io](https://github.com/ForceInjection/forceinjection.github.io) — distributed by [TomeVault](https://tomevault.io).
