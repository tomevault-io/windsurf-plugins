# mdxg-redline

> Source: [oubakiou/mdxg-redline](https://github.com/oubakiou/mdxg-redline) — distributed by [TomeVault](https://tomevault.io)

Inline review tool for MDXG markdown — capture highlighted comments and export them as structured JSON for LLM agents.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [oubakiou/mdxg-redline](https://github.com/oubakiou/mdxg-redline).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [oubakiou/mdxg-redline](https://github.com/oubakiou/mdxg-redline) — a repo with 9+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
