---
trigger: always_on
description: This repository is a Hugo theme with an example site used for development and validation. Theme templates live in `layouts/`, including partials, shortcodes, default render hooks, and page templates. Theme styles and generated assets live under `assets/theme/`, with SCSS in `assets/theme/scss/`. Content, configuration, translations, static files, and data for the demo site are in `exampleSite/`. Archetype templates are in `archetypes/`, install/setup helpers are in `scripts/`, and Hugo dependenc
---
