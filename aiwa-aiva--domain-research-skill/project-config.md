...
## 核心原则
> 先追时间深度，再追同期广度，最终交汇出判断。

你正在执行一次领域深度研究。最终产出一份**排版精美的PDF研究报告**。
...
拿到用户输入后，确认以下信息。如果用户已经给得足够明确（比如"帮我研究Hermes Agent"），不需要追问，直接开始：
...
- **子Agent 1 — 时间深度**：研究对象的起源、创始人背景、发展历程、关键事件、版本迭代、融资、战略转向、危机
- **子Agent 2 — 同期广度**：竞品识别、各竞品的特点和用户口碑、行业对比评测、市场份额
...
> - 如果用户环境中安装了 web-access skill（检查路径 `~/.claude/skills/web-access/SKILL.md` `~/.openclaw/skills/web-access/SKILL.md` `~/.hermes/skills/web-access/SKILL.md` 是否存在），优先加载它并遵循其指引，它提供更强的浏览器CDP能力
...
搜索完成后检查：
- 时间深度：能讲出一个完整的故事吗？有没有明显的信息断层？
- 同期广度：竞品列表完整吗？有没有遗漏主要玩家？每个竞品的信息够做对比吗？
...
这是整篇报告的精华段。把时间深度发展脉络和同期广度竞争格局结合起来，给出综合性的、新的判断。不要写成前面内容的缩写版。
...
这不是一份冷冰冰的咨询报告，而是一篇让人能从头读到尾的深度研究。写作风格需要具备「研究报告的严谨」。
...
1. **先完成Markdown稿件**：将完整报告写为标准Markdown格式，保存为 `[研究对象]_深度分析报告.md`

<!-- Content truncated to meet Windsurf 6KB limit -->

---
> Source: [aiwa-aiva/domain-research-skill](https://github.com/aiwa-aiva/domain-research-skill) — distributed by [TomeVault](https://tomevault.io).
