...
- Install package without build isolation (fast rebuilds):
  `uv sync --inexact --no-dev --no-build-isolation-package mqt-qcec`
...
- MUST regenerate stubs via `uvx nox -s stubs` when files in `bindings/` are
  added or modified.
- NEVER edit `.pyi` files in `python/mqt/qcec/` manually; they are
...
---
> Source: [munich-quantum-toolkit/qcec](https://github.com/munich-quantum-toolkit/qcec) — distributed by [TomeVault](https://tomevault.io).
