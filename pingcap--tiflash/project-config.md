---
trigger: always_on
description: This directory is the Rust FFI proxy for **next-gen columnar-only** TiFlash (`ENABLE_NEXT_GEN` + `ENABLE_NEXT_GEN_COLUMNAR`). It replaces `contrib/tiflash-proxy` / `contrib/tiflash-proxy-next-gen` in that build mode.
---
...
---
> Source: [pingcap/tiflash](https://github.com/pingcap/tiflash) — distributed by [TomeVault](https://tomevault.io).
