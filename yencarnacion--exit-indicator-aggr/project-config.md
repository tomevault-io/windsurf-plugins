---
trigger: always_on
description: - `server_py/` holds the FastAPI business logic: `app.py` wires routes, `state.py` tracks order book state, `depth.py` and `ib_client.py` wrap Interactive Brokers data. Config defaults live in `config.py` and `config.tws.yaml`.
---
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/yencarnacion)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/yencarnacion)
