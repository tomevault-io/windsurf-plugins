---
trigger: always_on
description: - Collector availability is controlled by Go build tags and OS-specific filenames; check `//go:build` lines before assuming a collector exists on the current OS.
---
...
- `end-to-end-test.sh` filters collector flags by OS/build context using `tools/tools match`, so an ignored flag in e2e may mean the collector is build-tag-gated for that platform rather than broken (see `Rules`).

## Toolchain Notes
- `go.mod`'s Go directive is not the source of truth for the toolchain version CI actually uses; check the workflow's `setup-go` step and `.promu.yml`/`.promu-cgo.yml`, which can each pin a different version.
...
---
> Source: [prometheus/node_exporter](https://github.com/prometheus/node_exporter) — distributed by [TomeVault](https://tomevault.io).
