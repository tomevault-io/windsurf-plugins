...
└── installer/
    ...
    ├── omp.rs       # Oh My Posh + Nerd Font（用户级字体注册 + profile 注入，卸载全逆向）
...
- 安装根目录：`{用户选的盘}:\hudo\`
  - `tools/` — CLI 工具与数据库（git, gh, rustup, uv, bun, mingw64, claude-code, mysql, pgsql, redis, fnm, miniconda）
  - `lang/` — 语言环境（go, java, node, cargo, gopath）
...
| 系统工具 | c（MinGW）, 7zip, pwsh, powertoys, omp, chrome, claude-code |
...
---
> Source: [zexadev/hudo](https://github.com/zexadev/hudo) — distributed by [TomeVault](https://tomevault.io).
