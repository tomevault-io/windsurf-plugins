---
trigger: always_on
description: Codex Runway 是一个原生 macOS 状态栏应用，用 Swift 编写，用于读取本机 Codex 配置和会话数据，展示配额、reset credits、API 等价成本，并修复本机会话索引。
---
...
- Codex Runway 自有缓存目录统一使用 `~/.codex-runway`，只保存非密钥的派生数据，不将应用缓存写入 `~/.codex`。
- 读取 `~/.codex/auth.json` 时只使用必要字段。
- 修改 `auth.json` 仅限 OAuth refresh 回写，必须原子写入。
