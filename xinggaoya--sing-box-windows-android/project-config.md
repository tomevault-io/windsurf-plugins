---
trigger: always_on
description: 本仓库为单模块 Android 应用，主模块在 `app/`。核心源码位于 `app/src/main/java/cn/moncn/sing_box_windows/`，按 `config/`、`core/`、`ui/`、`vpn/` 分包组织，入口为 `MainActivity.kt`。界面与资源在 `app/src/main/res/`，本地依赖库 `libbox.aar` 位于 `app/libs/`。测试分别在 `app/src/test/`（单元测试）与 `app/src/androidTest/`（仪器测试）。
---
