---
trigger: always_on
description: - `crates/`: Rust workspace crates — `server` (API + bins), `db` (SQLx models/migrations), `executors`, `services`, `utils`, `git` (Git operations), `api-types` (shared API types for local + remote), `review` (PR review tool), `deployment`, `local-deployment`, `remote`.
---
...
---
> Source: [cdesktop-ai/cdesktop](https://github.com/cdesktop-ai/cdesktop) — distributed by [TomeVault](https://tomevault.io).
