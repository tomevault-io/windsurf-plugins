---
trigger: always_on
description: This project is a jupyter client for Neovim, written in Rust and callable
---

## Project overview

This project is a jupyter client for Neovim, written in Rust and callable
in Neovim via Lua using the `mlua` crate in module mode.

## Testing

Integration tests are files like `lua_tests/test_*.lua`. You can run these
using `luajit`.

When you run you should probably ensure that all log messages are printed by
setting the environmental variable`RUST_LOG=trace`. Log messages are written to
`jet.log`.

If you need to do anything with Python, you should first create a virtual
environment to work in. You can do this with `uv`:

``` sh
uv venv
uv sync
source .venv/bin/activate
```

---
> Source: [wurli/jet](https://github.com/wurli/jet) — distributed by [TomeVault](https://tomevault.io).
<!-- tomevault:4.0:windsurf_rules:2026-05-05 -->
