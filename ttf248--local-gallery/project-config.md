---
trigger: always_on
description: > 本文件是仓库内 AI 助手（Claude / Codex / 其他）与人类协作者共同遵循的规则。
---
...
- 配置文件：
  ...
  - 缓存目录：`cacheDir` 未配置时自动在 CWD 下创建 `.local-gallery/`（存放缩略图、扫描结果、用户偏好）。
...
- **缓存目录**：未配置 `cacheDir` 时在进程 CWD 下创建 `.local-gallery/`（存放缩略图、扫描结果、用户偏好），不入仓；不要把缓存路径硬编码进源码，必须从 `config.yaml` 派生。
...
---
> Source: [ttf248/local-gallery](https://github.com/ttf248/local-gallery) — distributed by [TomeVault](https://tomevault.io).
