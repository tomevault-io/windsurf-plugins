# Seo-Promt-Master

> Source: [umutxyp/Seo-Promt-Master](https://github.com/umutxyp/Seo-Promt-Master). Graded, signed and kept current as the models change by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [umutxyp/Seo-Promt-Master](https://github.com/umutxyp/Seo-Promt-Master).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

## Bundled Skills (1)

- [Seo-Promt-Master](https://github.com/umutxyp/Seo-Promt-Master/tree/main/.claude/skills/seo-audit/SKILL.md)

From [umutxyp/Seo-Promt-Master](https://github.com/umutxyp/Seo-Promt-Master) — a repo with 555+ stars on GitHub.

---

Install this config instantly:
```
npx tomevault install umutxyp/Seo-Promt-Master
```
Source: [github.com/umutxyp/Seo-Promt-Master](https://github.com/umutxyp/Seo-Promt-Master).

<!-- genome:d-i-s -->
