---
trigger: always_on
description: This repository is a Chrome MV3 extension for linux.do. Source code lives in `src/`: `app/` contains the shared React UI, `side-panel/` and `options/` are extension entry points, `content/` injects linux.do page integrations, `background/` owns service-worker commands, `api/` and `domain/` hold parsing and business logic, and `state/` contains Jotai atoms. Static extension metadata is in `public/manifest.json`. Build output goes to `dist/`, release zips to `packages/`, and both are ignored. Proj
---
...
---
> Source: [LeUKi/linuxdo-friends](https://github.com/LeUKi/linuxdo-friends) — distributed by [TomeVault](https://tomevault.io).
