# claude-pty-wrapper

> Source: [kcosr/claude-pty-wrapper](https://github.com/kcosr/claude-pty-wrapper) — distributed by [TomeVault](https://tomevault.io)

claude-pty-wrapper runs interactive Claude through a PTY while exposing a print-like CLI that streams assistant text from Claude’s durable session JSONL.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [kcosr/claude-pty-wrapper](https://github.com/kcosr/claude-pty-wrapper).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/kcosr/claude-pty-wrapper](https://github.com/kcosr/claude-pty-wrapper)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-q -->
