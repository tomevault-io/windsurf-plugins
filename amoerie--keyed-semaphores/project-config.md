---
trigger: always_on
description: **KeyedSemaphores** is a C# library that provides per-key locking primitives to improve parallelism in multithreaded applications. Instead of a single global lock, callers acquire a lock scoped to a string key, so only threads sharing the same key are serialized.
---
...
---
> Source: [amoerie/keyed-semaphores](https://github.com/amoerie/keyed-semaphores) — distributed by [TomeVault](https://tomevault.io).
