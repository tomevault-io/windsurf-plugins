---
trigger: always_on
description: Versioned files under `spec/` define the Agent Plugins specification, with corresponding schemas under `schemas/` providing its machine-readable forms. The specification text is authoritative if it conflicts with a schema. The instructions below describe the design instincts and working practices that should shape proposed changes.
---
...
Versioned files under `spec/` define the Agent Plugins specification, with corresponding schemas under `schemas/` providing its machine-readable forms. The specification text is authoritative if it conflicts with a schema. The instructions below describe the design instincts and working practices that should shape proposed changes.
...
- Standardize behavior only when it can be implemented consistently across clients and serves a demonstrated portability need.
...
- Keep client policy, UX, presentation, and client-specific behavior outside the portable core.
...
- Evaluate proposals from the perspectives of plugin authors, client implementers, and client-specific innovation.
...
---
> Source: [agentplugins/agent-plugins-spec](https://github.com/agentplugins/agent-plugins-spec) — distributed by [TomeVault](https://tomevault.io).
