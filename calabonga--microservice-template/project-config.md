---
trigger: always_on
description: CQRS-операции в шаблоне оформлены как `public static class` с вложенными типами `Request` и `Handler` (паттерн `Mediator`, не MediatR — на верхнем уровне `Command`/`Handler`-типов нет).
---
