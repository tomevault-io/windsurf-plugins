---
trigger: always_on
description: Keep this file in sync with `CLAUDE.md`. `CLAUDE.md` imports this file, so edit this one.
---

# Agent instructions

Keep this file in sync with `CLAUDE.md`. `CLAUDE.md` imports this file, so edit this one.

## The project

OyasumiVR is a Windows desktop app for VRChat users, built with Tauri.

| Directory              | What it is                                        |
| ---------------------- | ------------------------------------------------- |
| `src-ui`               | Angular frontend (the main window)                |
| `src-core`             | Rust Tauri backend                                |
| `src-overlay-sidecar`  | C# sidecar, renders the SteamVR overlays          |
| `src-overlay-ui`       | SvelteKit frontend for the overlays               |
| `src-elevated-sidecar` | Rust sidecar for actions that need admin rights   |
| `src-shared-ts`        | TypeScript shared between the frontends           |
| `src-shared-rust`      | Rust shared between the core and elevated sidecar |

The core talks to the sidecars over gRPC. The `.proto` files live in `proto`.

## Comments

Follow `docs/COMMENT_POLICY.md`. Read it before you write or change a comment.

## Do not edit generated files

- `README.md` and `docs/readmes/generated`: generated by `npm run generate:readmes`. Edit
  `docs/readmes/src` instead.
- `src-grpc-web-client`: generated from `proto` by `npm run generate:grpc-web-client`.

## Translations

Add and change English strings only, in `src-ui/assets/i18n/en.json`. Translators fill in the other
locales. `npm run tl set` writes `en.json` only, which is what you want. To rename or remove a key,
use `npm run tl mv` or `npm run tl unset`, so the change reaches every locale file.

---
> Source: [Raphiiko/OyasumiVR](https://github.com/Raphiiko/OyasumiVR) — distributed by [TomeVault](https://tomevault.io).
<!-- tomevault:4.0:windsurf_rules:2026-08-16 -->
