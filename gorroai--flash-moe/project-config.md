...
### Perplexity (WikiText-2, 500 tokens)

| Configuration | PPL | tok/s |
|--------------|-----|-------|
| 4-bit experts | **5.51** | 10.16 |
| 2-bit experts | 8.09 | 12.36 |
| Mixed (per-layer 2/4-bit) | 8.22 | 12.98 |

PPL measured via `--ppl` flag using cross-entropy on ground truth tokens. Lower is better. The engine supports per-layer mixed quantization — specific layers can stay at 4-bit while others use 2-bit (`--skip-layers` in `repack_experts_2bit.py`).
...
---
> Source: [gorroai/flash-moe](https://github.com/gorroai/flash-moe) — distributed by [TomeVault](https://tomevault.io).
