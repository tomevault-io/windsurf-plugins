---
trigger: always_on
description: enableStrategySwitching: true,       // Auto-switch strategy on loop detection
---
...
- Conductor's Suite: Agent self-awareness via four meta-tools — `brief` (situational overview), `find` (unified search: RAG→memory→web auto-routing), `pulse` (reactive intelligence introspection: entropy, loops, context pressure, readiness), `recall` (selective working memory with droplet retrieval, replaces scratchpad). Built-in harness skill teaches the conductor's workflow. `controllerDecisionLog` on KernelState. `DocumentSpec.content` now optional (reads from `source` file path). `rag-search` source filter uses case-insensitive substring. `detectCompletionGaps` extracted to `@reactive-agents/tools`. `.withMetaTools()` builder API, `HarnessSkillConfig` for override/evolution (2,993 tests, 342 files)
- V1.0 Harness Optimization: Native function calling replaces text-based ACTION: parsing — `ProviderCapabilities` declarations on all providers, `ToolCallResolver` with `NativeFCStrategy` in `@reactive-agents/tools`, kernel FC branch (tools passed via API parameter, response.toolCalls read directly), `KernelMessage` provider-agnostic type, Anthropic `stream()` tools fix, `StepMetadata.toolCall` structured data, compaction/entropy/completion-guard FC-aware, harness skill slimmed (strategy guidance only, no tool format mechanics), conditional memory-flush (skip when no MemoryService), trivial task fast-path (1 iteration for simple Q&A), scratchpad→recall migration complete, entropy sensor short-run calibration (Grade A for 1-2 iteration successes), observation auto-forwarding (3,026 tests, 349 files)
...
bun test                 # Run all tests (3026 tests, 349 files)
...
const agent = await ReactiveAgents.create()
  ...
  .withMetaTools({                       // Conductor's Suite: brief, find, pulse, recall
    brief: true,                         // Situational awareness tool
    find: true,                          // Unified search (RAG→memory→web auto-routing)
    pulse: true,                         // Reactive intelligence introspection
    recall: true,                        // Selective working memory (replaces scratchpad)
    harnessSkill: true,                  // Inject built-in conductor's workflow skill
  })
...
| `docs/superpowers/specs/2026-03-24-conductors-suite-design.md` | Conductor's Suite (`brief`, `find`, `pulse`, `recall` meta-tools, harness skill, `.withMetaTools()`) |
| `docs/superpowers/specs/2026-03-26-v1-harness-optimization-design.md` | V1.0 Harness Optimization (native FC, ToolCallResolver, ProviderCapabilities, surgical efficiency fixes) |
| `docs/superpowers/specs/2026-03-28-intelligent-context-synthesis-design.md` | Intelligent Context Synthesis (ICS): `ContextSynthesizer`, kernel hooks, `.withReasoning()` synthesis + per-strategy overrides |
...
packages/
  ...
  llm-provider/  — LLM adapters (Anthropic, OpenAI, Gemini, Ollama, LiteLLM, Test); `ProviderCapabilities` declarations per provider
  ...
  reasoning/     — ReAct, Plan-Execute, ToT, Reflexion, Adaptive; ICS (`ContextSynthesizerService`, `classifyTaskPhase`, synthesis templates, `SynthesisConfig` threading)
  tools/         — Tool registry, sandbox, MCP client; Conductor's Suite: `recall`, `find`, `brief`, `pulse` meta-tools, `completion-gaps.ts` shared utility; Tool Calling: `ToolCallResolver`, `NativeFCStrategy`, `ProviderCapabilities`-driven strategy selection
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/tylerjrbuell)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/tylerjrbuell)
