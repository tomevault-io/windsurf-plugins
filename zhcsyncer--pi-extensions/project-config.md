---
trigger: always_on
description: - 维护中的用户可见包文档必须提供双语版本：英文使用默认 `README.md`，简体中文使用 `README.zh-CN.md`；只有仓库中明确记录的例外可以省略该规则。
---
...
- 一份 changeset 只有一段正文，会原样进入所列每个包的 CHANGELOG 和 GitHub Release。只把用户感知为同一件事的包写进同一份；根包和那个子包可以共用。旁路包若用户看到的是另一件事，另写一份，不要把主产品说明复制过去。

---
> Source: [zhcsyncer/pi-extensions](https://github.com/zhcsyncer/pi-extensions) — distributed by [TomeVault](https://tomevault.io).
