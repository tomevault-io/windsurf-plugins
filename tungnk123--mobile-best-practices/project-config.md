---
trigger: always_on
description: This file guides Claude Code (claude.ai/code) when working with this repository.
---
...
**Mobile Best Practices** is a searchable database of **1,937 mobile development best practices** packaged as an AI skill for Claude Code and other AI coding assistants. It covers architecture patterns, design patterns, UI components, anti-patterns, libraries, performance rules, security practices, testing patterns, code snippets, Gradle dependencies, and platform-specific guidelines for Android, iOS, Flutter, and React Native.
...
Flags: `--domain`/`-d` | `--platform`/`-p` | `--filter-platform`/`-fp` | `--stack`/`-s` | `--max-results`/`-n` | `--all-domains`/`-a` | `--fuzzy`/`-f` | `--compact`/`-c` | `--comment-style`/`-cs` | `--json` | `--persist` | `--page`
...
---
> Source: [tungnk123/mobile-best-practices](https://github.com/tungnk123/mobile-best-practices) — distributed by [TomeVault](https://tomevault.io).
