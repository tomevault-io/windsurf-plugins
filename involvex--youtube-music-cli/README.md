# youtube-music-cli

> Source: [involvex/youtube-music-cli](https://github.com/involvex/youtube-music-cli). Graded, signed and kept current as the models change by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `.cursor/rules/*.mdc` in [involvex/youtube-music-cli](https://github.com/involvex/youtube-music-cli).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/involvex/youtube-music-cli](https://github.com/involvex/youtube-music-cli)

---

Install this config instantly:
```
npx tomevault install involvex/youtube-music-cli
```
Source: [github.com/involvex/youtube-music-cli](https://github.com/involvex/youtube-music-cli).

<!-- genome:d-i-p -->
