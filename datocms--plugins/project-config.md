---
trigger: always_on
description: - `src/main.tsx` is the plugin entry. A single `connect()` registers the `zonedDateTimePicker` editor extension for `json` fields (`renderFieldExtension`) plus a `Show JSON value` field dropdown action that opens the debug modal (`renderModal`).
---
