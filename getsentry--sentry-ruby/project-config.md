---
trigger: always_on
description: [mise](https://mise.jdx.dev) manages Rubies and runs tasks. `.mise.toml` pins a single default Ruby for local work; `.mise.ci.toml` (loaded with `MISE_ENV=ci`) pins the full per-matrix Ruby set that CI, `bin/test`, and `bin/relock` resolve against.
---
...
---
> Source: [getsentry/sentry-ruby](https://github.com/getsentry/sentry-ruby) — distributed by [TomeVault](https://tomevault.io).
