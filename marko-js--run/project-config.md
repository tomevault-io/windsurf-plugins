---
trigger: always_on
description: `@marko/run` is the Marko application framework: file-based routing (`src/routes/**` with `+page.marko`, `+layout.marko`, `+handler`, `+middleware`), a Vite plugin, and deployment adapters. pnpm workspaces; primary development happens in `packages/run`, adapters live in `packages/adapters/*`.
---
...
---
> Source: [marko-js/run](https://github.com/marko-js/run) — distributed by [TomeVault](https://tomevault.io).
