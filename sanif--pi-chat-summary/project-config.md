---
trigger: always_on
description: Chat Summary is a Pi extension that keeps one concise, branch-aware cumulative
---

# Chat Summary Contributor Guide

## Project purpose

Chat Summary is a Pi extension that keeps one concise, branch-aware cumulative
session summary in the scrolling transcript. Preserve its trust boundary,
privacy-safe diagnostics, responsive rendering, prompt precedence, and existing
`/summary` behavior.

Project prompt and configuration files are trusted-project data. Never probe,
read, create, edit, open, copy, or delete project-scoped files while the project
is untrusted. Recheck trust immediately before project-scoped writes and after
asynchronous editing.

The hidden `/summary demo` command exists only to create deterministic screenshot
...
- npm package: `@sanif/pi-chat-summary`
- GitHub repository: `sanif/pi-chat-summary`
- recommended npm install: `pi install npm:@sanif/pi-chat-summary`
- recommended GitHub install: `pi install git:github.com/sanif/pi-chat-summary`
...
1. Confirm the intended version and inspect existing release surfaces:
   ...
   npm view @sanif/pi-chat-summary version
   npm view @sanif/pi-chat-summary dist-tags --json
   ```
...
5. Commit the release and create an annotated version tag only when explicitly
   ...
   git tag -a vX.Y.Z -m "Chat Summary vX.Y.Z"
   ```
...
---
> Source: [sanif/pi-chat-summary](https://github.com/sanif/pi-chat-summary) — distributed by [TomeVault](https://tomevault.io).
