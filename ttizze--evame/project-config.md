...
- **UI コンポーネント**: 
  ...
  - React 19
- **スタイリング**: Tailwind CSS
- **データベース**: Drizzle ORM を使用した PostgreSQL
- **認証**: better auth
- **国際化**: next-intl
...
データベースは Drizzle ORM を使用して管理されています：

- スキーマ定義: `next/drizzle/schema.ts`
- マイグレーション: `next/drizzle/migrations/`
- シードスクリプト: `next/drizzle/seed.ts`
...
export async function fetchNewProjectsWithPagination(
  ...
  // Drizzleを使用したデータ取得ロジック
  const data = await db.select().from(someModel);
  return { data };
}
```
...
---
> Source: [ttizze/evame](https://github.com/ttizze/evame) — distributed by [TomeVault](https://tomevault.io).
