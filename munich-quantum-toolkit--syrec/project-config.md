...
- Install package without build isolation (fast rebuilds):
  `uv sync --inexact --no-dev --no-build-isolation-package mqt-syrec`
...
- MUST regenerate stubs via `uvx nox -s stubs` when files in `bindings/` are
  added or modified.
- NEVER edit `.pyi` files in `python/mqt/syrec/` manually; they are
...
---
> Source: [munich-quantum-toolkit/syrec](https://github.com/munich-quantum-toolkit/syrec) — distributed by [TomeVault](https://tomevault.io).
