---
trigger: always_on
description: CI（`.github/workflows/ci.yml`）会跑这四样：`pnpm build` → `cargo fmt --check` → `cargo check --all-targets --locked` → `cargo clippy --all-targets --locked -- -D warnings`。任一失败就红。**`git push` 之前必须本地复跑同一组，确认全绿再推**，否则等于把 CI 当 linter，浪费一轮 runner 时间。
---
...
---
> Source: [zuoliangyu/wos-fetch](https://github.com/zuoliangyu/wos-fetch) — distributed by [TomeVault](https://tomevault.io).
