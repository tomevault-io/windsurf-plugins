# ambitious

> Source: [scrogson/ambitious](https://github.com/scrogson/ambitious) — distributed by [TomeVault](https://tomevault.io)

A native Rust implementation of Erlang/OTP primitives, bringing the power of the BEAM's concurrency model to Rust with full type safety.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [scrogson/ambitious](https://github.com/scrogson/ambitious).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/scrogson/ambitious](https://github.com/scrogson/ambitious)

---

Install this config instantly:
```
npx tomevault install scrogson/ambitious
```
Source: [github.com/scrogson/ambitious](https://github.com/scrogson/ambitious).

<!-- genome:d-i-q -->
