# bitkit-android

> Tome by [synonymdev](https://github.com/synonymdev/bitkit-android) — distributed by [TomeVault](https://tomevault.io)

Self-custodial Bitcoin and Lightning Wallet

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [synonymdev/bitkit-android](https://github.com/synonymdev/bitkit-android).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/synonymdev/bitkit-android](https://github.com/synonymdev/bitkit-android)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:t-c-p -->
