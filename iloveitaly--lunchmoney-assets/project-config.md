...
* Use `log.info("the message", the_variable=the_variable)` instead of `log.info("The message: %s", the_variable)` or `print` for logging.
  * Use `structlog-config` for configuration: `from structlog_config import configure_logger; log = configure_logger()`
...
* Prefer union syntax `str | None` over `Optional[str]` or `Union[str, None]`.
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/iloveitaly)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/iloveitaly)
