...
- **Always** begin your **LiveView** templates with `<Layouts.app flash={@flash} current_scope={@current_scope} socket={@socket} show_sms_assistant_intro_modal={@show_sms_assistant_intro_modal} ...>` which wraps all inner content (controller templates omit **`socket`** / **`show_sms_assistant_intro_modal`**; they default to no intro modal). Workspace nav includes **Support** at **`/support`** (phone + email).
...
---
> Source: [151henry151/romp-crm](https://github.com/151henry151/romp-crm) — distributed by [TomeVault](https://tomevault.io).
