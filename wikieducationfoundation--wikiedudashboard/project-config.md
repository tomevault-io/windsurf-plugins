---
trigger: always_on
description: bundle exec rspec spec/path/to/spec.rb   # single spec file
---
...
By default, we never want AI-created or re-written text that users will see. Anything more substantive than a one-or-two-word button label should come from the operator, not generated by the AI. Ask for appropriate messaging any time user-facing text is needed; the AI should only write it if explicitly directed to do so at the individual message level.

When user-facing text is needed but the operator hasn't supplied it yet, leave a
`[PLACEHOLDER - <description of the copy needed>]` marker in its place instead of
