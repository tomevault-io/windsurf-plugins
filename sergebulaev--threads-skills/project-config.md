...
# Project conventions - threads-skills
...
- Keep `AGENTS.md` and `CLAUDE.md` aligned when changing shared project rules.
  ...
  `CLAUDE.md`.
- Codex marketplace install uses `.codex-marketplace/threads-skills/`. Do not
...
- **Skill names are public surface.** Renaming a skill is a major version bump and
  ...
  bundle list, README skill table, and every `threads-<name>` cross-reference in
...
- Other root-level references: `references/hook-formulas.md` (10 Threads
...
- `threads-humanizer` has `sub-skills/` for the folded-in post-audit workflow.

## Layer separation

- **Write layer (Publora):** `lib/publora_client.py`. `create_post` (single post
  or thread; long content auto-splits into a connected multi-post thread, no
  `(1/N)` markers) and `list_connections` / `threads_connections` (GET
  /platform-connections). Skills call `lib.publish(kind, draft_text,
  target_url, ...)`. Real endpoint: `POST /create-post` with
  `platforms: ["threads-<id>"]` (array of STRING ids), header `x-publora-key`.
  Threads `platformSettings.threads.replyControl` passes through
  `platform_settings`.
- **Threads has no comment/reply/reaction endpoint on Publora**, and
  `create-post` cannot target another user's post. `kind="reply"` always routes
  to a manual copy-paste block.
- **No read layer ships by default.** `threads-hook-extractor` and
  `threads-reply-drafter` ask the user to paste post text. Any future post-read
  actor stays gated behind `APIFY_TOKEN` with a paste fallback.
...
- `.agents/plugins/marketplace.json` points to `.codex-marketplace/threads-skills`.
...
---
> Source: [sergebulaev/threads-skills](https://github.com/sergebulaev/threads-skills) — distributed by [TomeVault](https://tomevault.io).
