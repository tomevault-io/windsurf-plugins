---
trigger: always_on
description: SkyRL is a full-stack reinforcement learning library for training LLMs, designed for modularity and
---

# SkyRL-IsoExec

SkyRL is a full-stack reinforcement learning library for training LLMs, designed for modularity and
extensibility. This fork adds **IsoExec**: bitwise-identical token logprobs between the Megatron
trainer and the vLLM rollout engine. See `skyrl/backends/skyrl_train/isoexec/README.md`.
...
# IsoExec contract and manifest tests (they live beside the package, not under tests/)
uv run --extra dev pytest skyrl/backends/skyrl_train/isoexec/
...
uv run --isolated --extra dev --extra megatron pytest tests/backends/skyrl_train/gpu/gpu_ci/megatron/test_megatron_worker.py
...
| IsoExec: layout, contract, flags, how to run it | `skyrl/backends/skyrl_train/isoexec/README.md` |
| The pinned torch / vLLM / triton / FA3 binaries IsoExec resolves | `pinned-wheels/README.md` |
...
---
> Source: [zanderjiang/SkyRL-IsoExec](https://github.com/zanderjiang/SkyRL-IsoExec) — distributed by [TomeVault](https://tomevault.io).
