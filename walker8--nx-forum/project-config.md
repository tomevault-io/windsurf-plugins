---
trigger: always_on
description: - `nx-forum-backend/`: Spring Boot 3 backend using COLA-style modules. Forum business code is in `nx-biz-forum/` with `adapter`, `app`, `domain`, `infrastructure`, and `start` modules. Shared platform modules live under `nx-platform/`.
---
