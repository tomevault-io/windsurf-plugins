---
trigger: always_on
description: telegram, digest, aiogram, telethon, openai, postgres, etl
---

# GRACE Framework — Project Engineering Protocol

## Keywords
telegram, digest, aiogram, telethon, openai, postgres, etl

## Annotation
Telegram bot that stores per-user channel lists and generates per-channel text digests from recent posts.
...
In `docs/*.xml` files, every **repeated entity** must use its **unique ID as the XML tag name** instead of a generic type tag with an `ID` attribute. This eliminates closing-tag polysemy.
...
### What NOT to change
- **CrossLinks** — self-closing (`<CrossLink ... />`)
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/TerrniT) — claim your Tome and manage your conversions.
