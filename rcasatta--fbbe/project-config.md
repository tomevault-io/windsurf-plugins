---
trigger: always_on
description: `src/` contains the application crate. HTTP entrypoints live in `src/main.rs` and `src/lib.rs`; request routing is in `src/route.rs`; page handlers are under `src/pages/`; HTML helpers are in `src/render/`; Bitcoin Core REST calls are grouped in `src/rpc/`; background sync/indexing tasks live in `src/threads/`. Static assets such as CSS and the favicon are also kept in `src/`. Integration-style tests are in `tests/e2e.rs`; smaller async and utility checks are in `tests/unit.rs`. Docker and Nix p
---
...
---
> Source: [RCasatta/fbbe](https://github.com/RCasatta/fbbe) — distributed by [TomeVault](https://tomevault.io).
