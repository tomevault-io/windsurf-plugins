---
trigger: always_on
description: This repo orchestrates lm-evaluation-harness runs against vLLM via YAML workloads in `workloads/`. `lib/run.sh` parses a workload, brings up vLLM in Docker, and dispatches each task to a helper in `lib/`. Real runs need GPUs and are exercised on Buildkite.
---
...
---
> Source: [vllm-project/perf-eval](https://github.com/vllm-project/perf-eval) — distributed by [TomeVault](https://tomevault.io).
