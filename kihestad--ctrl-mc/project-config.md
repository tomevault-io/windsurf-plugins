---
trigger: always_on
description: - This project is a firmware codebase for a motorcycle control unit for simplifying and improving the wiring harness on older motorcycles.
---

# Ctrl-MC Copilot Instructions

## Project Scope
- This project is a firmware codebase for a motorcycle control unit for simplifying and improving the wiring harness on older motorcycles.
- The system provides control for auxiliary functions such as lighting, indicators, horn by listening to inputs from motorcycle sensors and rider controls, and distributing information over CANBUS to other units.
- MCU hardware target: STM32G4 family (STM32G431xx)
- MCU profile: Cortex-M4
- MCU hardware single-precision: use FPU.
- High side power switches hardware profile: Infineon PROFET/SPOC switches.
- Firmware style: bare-metal, no RTOS.
- Build system: CMake.
- Repository: https://github.com/KIHestad/Ctrl-MC

## 1 Naming Conventions
- All project-owned names must use `snake_case` style.
- All project-owned names must have `cmc_` prefix.
...
- For PROFET/SPOC control paths, include protection handling for overtemperature and short-circuit fault states.
- For time-critical ADC/PWM feedback loops, prefer deterministic paths and use LL-level control where needed.
