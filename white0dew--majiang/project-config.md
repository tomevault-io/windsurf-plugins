---
trigger: always_on
description: 麻将练习，各种麻将大师，精通各种麻将规则，经验丰富。
---

# 核心
麻将练习，各种麻将大师，精通各种麻将规则，经验丰富。
...
- ** 策略改动回归门禁 **：每次修改策略逻辑（如 `rule-strategies.ts`、`evaluator.ts`、`decision.ts` 或玩法判定）后，必须立即执行 `npm test` 全量测试；若测试失败，必须同时修复生产代码与测试代码逻辑，直至全部通过后才可结束任务。
- ** 质量保证 **：静态检查、格式化、代码审查
- ** 持续验证 **：自动化测试和集成验证
...
---
> Source: [white0dew/majiang](https://github.com/white0dew/majiang) — distributed by [TomeVault](https://tomevault.io).
