---
trigger: always_on
description: - Prefer `uv run` to run Python (`uv run script.py`, `uv run -m module_name`, `uv run python -c "..."`), or activate `.venv` first. Do not use the system Python directly.
---
...
---
> Source: [NVIDIA/warp](https://github.com/NVIDIA/warp) — distributed by [TomeVault](https://tomevault.io).
