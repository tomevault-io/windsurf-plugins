# ClipBrain

> Source: [agentpilled/ClipBrain](https://github.com/agentpilled/ClipBrain) — distributed by [TomeVault](https://tomevault.io)

Your AI never starts from zero. Capture everything you read with Cmd+Shift+S.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [agentpilled/ClipBrain](https://github.com/agentpilled/ClipBrain).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [agentpilled/ClipBrain](https://github.com/agentpilled/ClipBrain) — a repo with 23+ stars on GitHub.

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-s -->
