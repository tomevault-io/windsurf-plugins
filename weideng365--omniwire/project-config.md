---
trigger: always_on
description: 本文件为 Claude Code (claude.ai/code) 在此仓库中工作时提供指导，所有对话必须是中文回答。
---
...
7. **Go embed 必须使用 `all:` 前缀**：Go 的 `//go:embed` 指令默认跳过 `_` 和 `.` 开头的文件，而 Vite 构建会生成 `_plugin-vue_export-helper-xxx.js` 等以 `_` 开头的文件。必须使用 `//go:embed all:public` 而非 `//go:embed public`，否则前端资源加载失败。
8. **前端打包后必须同步到 packed 目录**：前端构建输出到 `server/resource/public`，但 embed 嵌入的是 `server/internal/packed/public`。编译 Linux 二进制前，必须先将 `resource/public` 同步到 `internal/packed/public`，否则嵌入的是旧资源。
9. **新增页面必须同步修改 MainLayout.vue 菜单**：在 `router/index.js` 添加路由后，必须同时在 `layouts/MainLayout.vue` 的 `el-menu` 中添加对应菜单项，否则侧边栏不会显示新页面入口。
...
---
> Source: [weideng365/OmniWire](https://github.com/weideng365/OmniWire) — distributed by [TomeVault](https://tomevault.io).
