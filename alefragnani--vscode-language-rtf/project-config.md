...
- ✅ Nested properties preserved (e.g., `"rtf.configuration.someConfig.description"`)
...
- ✅ Command links intact: `(command:rtf.whatsNew)`
...
1. **Missing keys** - Incomplete localization
   ```
   Missing keys: ["rtf.commands.whatsNew.title"]
   ```
...
1. **Extra keys** - May indicate stale translations
   ```
   Extra keys not in base file: ["rtf.commands.oldCommand.title"]
   ```

2. **Untranslated values** - Values identical to English
   ```
   "rtf.commands.newCommand.title": "New Command"  // Appears untranslated
   ```
