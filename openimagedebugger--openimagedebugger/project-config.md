---
trigger: always_on
description: Always format code using clang-format after editing.
---

# Project Instructions for OpenImageDebugger

## Formatting Instructions

Always format code using clang-format after editing.

## Checking CI Logs

Use the `gh` client outside of a sandbox when requested to check CI logs.
...
- **IMPORTANT**: When starting a new task, you MUST disable automatic analysis with the `toggle_automatic_analysis` tool if it exists. Always double check this.
- **IMPORTANT**: When you are done generating code at the very end of the task, you MUST re-enable automatic analysis with the `toggle_automatic_analysis` tool if it exists. Always double-check this.
...
---
> Source: [OpenImageDebugger/OpenImageDebugger](https://github.com/OpenImageDebugger/OpenImageDebugger) — distributed by [TomeVault](https://tomevault.io).
