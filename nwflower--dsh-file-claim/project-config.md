---
trigger: always_on
description: `dsh-file-claim` 是 DeepSeek Harness 的 Host 插件：为**同一工作区并行多会话**提供文件认领/保护（claim/release、心跳 stale 接管、pending 异步三路合并区）。DSH 的哲学是 **everything is a plugin**——本仓库只做插件，不碰引擎。改代码前先读 `README.md`（对外契约）、`dev/REQUIREMENTS.md`（需求清单）与 `dev/file-protection-plugin-study.md`（立项评估）。
---
...
---
> Source: [Nwflower/dsh-file-claim](https://github.com/Nwflower/dsh-file-claim) — distributed by [TomeVault](https://tomevault.io).
