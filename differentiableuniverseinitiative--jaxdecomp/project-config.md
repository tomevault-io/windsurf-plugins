---
trigger: always_on
description: This project, `jaxDecomp`, is a JAX library for performing 3D domain decomposition and parallel Fast Fourier Transforms (FFTs). It provides a high-level API for these operations, with two backend implementations: a pure JAX backend and a high-performance backend that uses NVIDIA's cuDecomp library. The pure JAX backend is recommended for ease of use and does not require MPI, while the cuDecomp backend offers better performance on multi-node GPU systems.
---
...
---
> Source: [DifferentiableUniverseInitiative/jaxDecomp](https://github.com/DifferentiableUniverseInitiative/jaxDecomp) — distributed by [TomeVault](https://tomevault.io).
