---
trigger: always_on
description: 一切以“正确”为原则；当旧实现明显不合理时，应优先选择更正确、更合理、更易理解的方案。同时，改动范围仍应服务于当前目标，不要引入不必要的复杂性。涉及版本语义、发布通道、自动升级、持久化数据、数据库 schema 或迁移逻辑时，必须明确兼容策略、迁移路径和验证方式，避免破坏已有安装和用户数据。
---
