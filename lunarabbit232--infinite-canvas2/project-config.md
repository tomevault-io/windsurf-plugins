...
- 本机配置有限（约 15GB 内存），严禁安装或运行 Docker；此前多次尝试 Docker 部署导致系统内存耗尽死机。
- 严禁安装或使用 Bun（包括 `bun install`、`bun run` 等）；Dockerfile 中引用的 `oven/bun:1.3.14` 已知会导致运行时崩溃。本地开发和运行优先使用 Node.js + npm。
- 本地运行方式：直接使用已编译的 `infinite-canvas-server` 二进制（后端 8080）+ `next dev --webpack`（前端 3000），参考 `web/dev.sh`。不要试图用 Docker 或 Bun 构建/运行。

---
> Source: [lunarabbit232/infinite-canvas2](https://github.com/lunarabbit232/infinite-canvas2) — distributed by [TomeVault](https://tomevault.io).
