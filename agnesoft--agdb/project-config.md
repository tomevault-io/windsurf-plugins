---
trigger: always_on
description: The Agnesoft Graph Database (aka agdb) is a graph database. The main components of this repository are `agdb` (rust package) that is the database itself. The `agdb_server` (rust package) is the server version of the database. API client packages are available for Rust, TypeScript, and PHP. The OpenAPI specification is located at `agdb_server/openapi.json`.
---
...
---
> Source: [agnesoft/agdb](https://github.com/agnesoft/agdb) — distributed by [TomeVault](https://tomevault.io).
