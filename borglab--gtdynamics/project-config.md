...
* `gtdynamics.i` uses `gtwrap` interface syntax (not SWIG). Follow the existing grammar in that file, e.g., inheritance is written as `class Derived : Base` (no `public` keyword).
...
---
> Source: [borglab/GTDynamics](https://github.com/borglab/GTDynamics) — distributed by [TomeVault](https://tomevault.io).
