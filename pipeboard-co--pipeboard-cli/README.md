# pipeboard-cli

> Source: [pipeboard-co/pipeboard-cli](https://github.com/pipeboard-co/pipeboard-cli) — distributed by [TomeVault](https://tomevault.io)

Command-line tool for managing Meta Ads, Google Ads, and TikTok Ads from your terminal. Create campaigns, pull reports, and manage ads across platforms with a single CLI.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [pipeboard-co/pipeboard-cli](https://github.com/pipeboard-co/pipeboard-cli).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/pipeboard-co/pipeboard-cli](https://github.com/pipeboard-co/pipeboard-cli)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-q -->
