# Selflify

> Source: [Selflify/Selflify](https://github.com/Selflify/Selflify) — distributed by [TomeVault](https://tomevault.io)

Free and open source self-hosted preview deploy control for static SPAs.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [Selflify/Selflify](https://github.com/Selflify/Selflify).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [Selflify/Selflify](https://github.com/Selflify/Selflify) — a repo with 9+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
