...
| `test/e2e/` | E2E tests suite (Ginkgo/CAPI framework) |
...
| `make test-e2e` | E2E tests (bootstraps Kind cluster and runs basic IPAM operations) |
...
Standalone E2E tests for IPAM live in `test/e2e/` as a separate Go module.
They use the CAPI test framework to bootstrap a Kind cluster with CAPI + IPAM providers.

| Command | Purpose |
|---------|---------|
| `.hack/e2e/ci-script.sh` | Full E2E suite |
| `GINKGO_FOCUS_LABELS=features ./hack/e2e/ci-script.sh` | Run specific test |
| `USE_EXISTING_CLUSTER=true SKIP_RESOURCE_CLEANUP=true ./hack/e2e/ci-script.sh` | Run against an existing cluster with out cleanup to debug |

Key files:

- `test/e2e/config/e2e_conf.yaml` - Provider versions and test config
- `hack/e2e/ci-script.sh` - CI orchestration script

See [docs/e2e.md](docs/e2e.md) for full details.

Integration-level E2E also runs through
...
---
> Source: [metal3-io/ip-address-manager](https://github.com/metal3-io/ip-address-manager) — distributed by [TomeVault](https://tomevault.io).
