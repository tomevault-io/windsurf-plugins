---
trigger: always_on
description: This file provides instructions for AI coding agents to effectively work on this project.
---
...
Mock implementations are auto-generated by [mockery](https://github.com/vektra/mockery) from domain interfaces. **Do NOT hand-edit files in `internal/testutil/mock/` that start with `mock_`** — they will be overwritten.

-   **Regenerate mocks** (after changing any interface in `domain/`):
    ```bash
    make mock
    ```
-   **Configuration**: See `.mockery.yaml` for the list of interfaces being mocked.
-   **Adding a new interface**: Add it to `.mockery.yaml` under the appropriate package, then run `make mock`.
-   **Custom helpers**: Hand-written test helpers live in `internal/testutil/mock/helpers.go` (e.g., `NewPassthroughTxManager`).
...
---
> Source: [kirklin/boot-backend-go-clean](https://github.com/kirklin/boot-backend-go-clean) — distributed by [TomeVault](https://tomevault.io).
