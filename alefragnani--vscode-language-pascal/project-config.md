...
- **Example**: `codeNavigation.nls.es.md`, `formatter.nls.pt-br.md`
...
- ✅ Nested properties preserved (e.g., `"pascal.configuration.tags.autoGenerate.description"`)
...
- ✅ Command links intact: `(command:pascal.generateTags)`
...
1. **Missing keys** - Incomplete localization
   ```
   Missing keys: ["pascal.commands.newFile.title"]
   ```
...
5. **Incomplete localization pack** - Missing files for a new language
   ...
   - Walkthrough `.nls.pt-br.md` files
   ```
...
1. **Extra keys** - May indicate stale translations
   ```
   Extra keys not in base file: ["pascal.commands.oldCommand.title"]
   ```
...
---
> Source: [alefragnani/vscode-language-pascal](https://github.com/alefragnani/vscode-language-pascal) — distributed by [TomeVault](https://tomevault.io).
