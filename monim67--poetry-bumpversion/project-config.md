---
trigger: always_on
description: `poetry-bumpversion` is a Poetry application plugin that extends the `poetry version` command to update version strings in arbitrary project files (e.g. `__init__.py`, `README.md`) whenever `poetry version <bump>` is run. It is published to PyPI and used via `poetry self add poetry-bumpversion`.
---
...
---
> Source: [monim67/poetry-bumpversion](https://github.com/monim67/poetry-bumpversion) — distributed by [TomeVault](https://tomevault.io).
