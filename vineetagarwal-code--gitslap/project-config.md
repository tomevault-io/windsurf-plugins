...
**gitslap** is a macOS CLI tool that detects physical hits/slaps on Apple Silicon MacBooks via the accelerometer and plays audio responses. Single-file Go application with embedded MP3 assets.
...
# Run (requires sudo)
sudo ./gitslap
sudo ./gitslap --sexy      # escalating responses mode
sudo ./gitslap --halo      # Halo death sounds mode
sudo ./gitslap --custom /path/to/mp3s  # custom audio directory
```

### Install

```bash
go install github.com/taigrr/gitslap@latest
```
...
---
> Source: [VineeTagarwaL-code/GitSlap](https://github.com/VineeTagarwaL-code/GitSlap) — distributed by [TomeVault](https://tomevault.io).
