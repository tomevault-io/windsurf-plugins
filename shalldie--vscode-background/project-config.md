...
1. `PatchGenerator.create(config)` 把用户配置编译成一段 JS 代码（多个 IIFE 拼接，最后用 `terser` 压缩）。
...
图片来源处理（在基类构造函数中）支持：`https://`、`data:`、本地文件、本地目录（用 `fast-glob` 扫支持的扩展名）；本地路径先经 `expandPathVariables` 展开 `~`（用户目录）和环境变量（`${ENV}`、`$ENV`），再被归一化为 `vscode-file://vscode-app/...` 协议（v1.51.1+ 的沙盒访问限制需要）。
...
---
> Source: [shalldie/vscode-background](https://github.com/shalldie/vscode-background) — distributed by [TomeVault](https://tomevault.io).
