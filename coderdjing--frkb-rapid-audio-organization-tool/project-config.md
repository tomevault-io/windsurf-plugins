---
trigger: always_on
description: - `src/main/` Electron main process (app lifecycle, native bridges, IPC handlers).
---
...
- 新增或修改 Renderer UI 时必须同时适配并检查 `theme-light` 和 `theme-dark`；禁止只按暗色或只按亮色交付。颜色、背景、边框、阴影、伪元素、canvas 绘制、worker 绘制都应优先走主题变量。日间模式下所有波形区域必须使用统一灰色底色变量（如 `--waveform-bg` / `--waveform-bg-elev`），避免 RGB 波形直接落在白底上。
...
---
> Source: [coderDJing/FRKB_Rapid-Audio-Organization-Tool](https://github.com/coderDJing/FRKB_Rapid-Audio-Organization-Tool) — distributed by [TomeVault](https://tomevault.io).
