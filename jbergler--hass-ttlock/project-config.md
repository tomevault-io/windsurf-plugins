---
trigger: always_on
description: A Home Assistant custom integration (HACS) for TTLock smart locks. Domain: `ttlock`. Code lives entirely under `custom_components/ttlock/` as flat files — no subpackages. Keep it that way unless a restructure is explicitly requested; don't introduce a `helpers/`, `utils/`, or similar catch-all package.
---
...
---
> Source: [jbergler/hass-ttlock](https://github.com/jbergler/hass-ttlock) — distributed by [TomeVault](https://tomevault.io).
