# ClawXMemory

> Source: [OpenBMB/ClawXMemory](https://github.com/OpenBMB/ClawXMemory) — distributed by [TomeVault](https://tomevault.io)

ClawXMemory: A Multi-Level Memory Plugin for OpenClaw with Long-Term Context

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [OpenBMB/ClawXMemory](https://github.com/OpenBMB/ClawXMemory).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [OpenBMB/ClawXMemory](https://github.com/OpenBMB/ClawXMemory) — a repo with 29+ stars on GitHub.

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-s -->
