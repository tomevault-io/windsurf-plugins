# OpenLens

> Source: [martynpekala/OpenLens](https://github.com/martynpekala/OpenLens) — distributed by [TomeVault](https://tomevault.io)

iOS companion app for OpenCode- chat with your AI coding assistant from your phone

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [martynpekala/OpenLens](https://github.com/martynpekala/OpenLens).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [martynpekala/OpenLens](https://github.com/martynpekala/OpenLens) — a repo with 9+ stars on GitHub.

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-s -->
