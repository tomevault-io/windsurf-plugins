...
- Sempre use React Hook Form para criação e validação de formulários. SEMPRE use o componente [form.tsx](mdc:components/ui/form.tsx) e veja os componentes [sign-in-form.tsx](mdc:app/authentication/components/sign-in-form.tsx) e [sign-up-form.tsx](mdc:app/authentication/components/sign-up-form.tsx) para ter uma base de como fazer.
...
- As Server Actions devem ser armazenadas em `actions` (siga o padrão de nomenclatura das já existentes). Cada server action deve ficar em uma pasta com dois arquivos: index.ts e schema.ts. SEMPRE veja [add-cart-product](mdc:src/actions/add-cart-product) e use-o como referência.
- Sempre que for necessário interagir com o banco de dados, use o [index.ts](mdc:_db/index.ts) e veja o arquivo [schema.ts](mdc:src/db/schema.ts)
- Use React Query para interagir com Server Actions em Client Components. SEMPRE use os componentes [cart-item.tsx](mdc:src/components/common/cart-item.tsx) [cart.tsx](components/cart.tsx) como exemplo.
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/duduzinmuller)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/duduzinmuller)
