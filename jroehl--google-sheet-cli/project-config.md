...
**Merge `fix/2.3.0` and `modernize` with a real merge commit, never a squash.** `v2.2.0` is not an ancestor of `master` - the tag history and the branch history diverged before 2.2.0 - and `fix/2.3.0` carries a `-s ours` merge that links it back. A squash discards that merge parent, so the newest tag reachable from `master` stays `v2.1.4` and `semantic-release` computes the next version from 2.1.4 rather than 2.2.0. Check with `git merge-base --is-ancestor v2.2.0 master` after merging: it must exit 0. The sibling `gsheet.action` repository requires the opposite - squash there, because its branches carry trailers that must not reach public history - so do not carry the habit across.

Version lines: `latest` is 3.x and needs Node 22; the 2.x line stays installable as `google-sheet-cli@^2` and is maintained on the `2.x` branch. There is deliberately no `v2` dist-tag: publishing is OIDC trusted publishing with no npm token anywhere, so a dist-tag could only be moved by hand on every 2.x release, and a semver range resolves to the newest 2.x on its own. The order of operations for the 3.0.0 publish and the `2.x` branch's own configuration are in `test-docs/revive-v3.md` under *Step 16* and *Step 13*. All of it is the repository owner's, and each push needs their confirmation for that specific push.
...
---
> Source: [jroehl/google-sheet-cli](https://github.com/jroehl/google-sheet-cli) — distributed by [TomeVault](https://tomevault.io).
