---
trigger: always_on
description: **Language / 对人说话：** Match the user's latest message. English in → English out. Chinese in → Chinese out. Do not mix. Slash names and `config.json` keys stay as written. Script tables (doctor / analyse) stay Chinese; prepend one sentence in the user's language. See `.cursor/rules/skills-plain-language.mdc`.
---
