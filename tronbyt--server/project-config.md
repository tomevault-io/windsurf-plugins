---
trigger: always_on
description: This project is the **Tronbyt Server**, a Go-based application which manages Tronbyt devices (flashed Tidbyts) locally, providing a web UI and API for app management and rendering.
---
...
*   **Database:** Use GORM for DB interactions. Ensure migrations are updated in `cmd/migrate` or auto-migrated in `server.go` if appropriate.
    *   **Generics API:** Strongly prefer the new [Generics API](https://gorm.io/docs/the_generics_way.html) (e.g., `gorm.G[data.User](db).First(...)`) over the old interface-based API (`db.Model(&data.User{}).First(...)`) for better type safety and clarity.
...
---
> Source: [tronbyt/server](https://github.com/tronbyt/server) — distributed by [TomeVault](https://tomevault.io).
