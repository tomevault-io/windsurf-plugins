---
trigger: always_on
description: dsh-auth-gateway 是 dsh web 前面的密码 + TOTP 认证网关（Cordis 插件）：网关独占对外端口，bundle patch 把内部 webserver 钉在 `127.0.0.1:<N+1>`，每个 HTTP 请求与 WebSocket 升级先过认证门再转发。ESM（`"type": "module"`）、Node >= 20。改 `lib/` 前先读 [docs/zh/DEVELOPMENT.md](docs/zh/DEVELOPMENT.md)；安全语义见 [docs/zh/SECURITY.md](docs/zh/SECURITY.md)。
---
...
---
> Source: [xbzbing/dsh-auth-gateway](https://github.com/xbzbing/dsh-auth-gateway) — distributed by [TomeVault](https://tomevault.io).
