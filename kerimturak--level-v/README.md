# level-v

> Source: [kerimturak/level-v](https://github.com/kerimturak/level-v) — distributed by [TomeVault](https://tomevault.io)

LEVEL: Open-source RV32IMC RISC-V processor core with pipelined microarchitecture, cache support, SoC peripherals and verification framework.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `copilot-instructions.md` in [kerimturak/level-v](https://github.com/kerimturak/level-v).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **Codex** — `AGENTS.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/kerimturak/level-v](https://github.com/kerimturak/level-v)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-p -->
