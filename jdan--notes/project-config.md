---
trigger: always_on
description: - Install with `npm install`; this repo uses `package-lock.json`.
---
...
- When editing draft Notion pages, you can use `NOTION_SECRET` with the Notion API to inspect and modify blocks directly; for Val Town embeds, use the Val Town MCP to read the referenced val source and replace the embed block with a Notion `code` block when requested. Ask whether to also add useful output blocks for examples and format code blocks for mobile readability, commonly with `oxfmt` at `printWidth: 40`, `useTabs: false`, and `indentWidth: 2`, trimming `Open in Val Town` headers and trailing newlines.
...
---
> Source: [jdan/notes](https://github.com/jdan/notes) — distributed by [TomeVault](https://tomevault.io).
