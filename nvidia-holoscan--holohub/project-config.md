---
trigger: always_on
description: When reviewing Pull Requests that modify or add `metadata.json` files, verify that the **category** (first tag in the `tags` array) of those `metadata.json` files is correctly set and matches one of the existing categories below.
---
...
---
> Source: [nvidia-holoscan/holohub](https://github.com/nvidia-holoscan/holohub) — distributed by [TomeVault](https://tomevault.io).
