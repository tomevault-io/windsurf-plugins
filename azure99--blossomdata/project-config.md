---
trigger: always_on
description: - `src/blossom/` is the core library. Major areas: `context/`, `dataset/`, `dataframe/`, `op/`, `provider/`, `schema/`, `util/`.
---
...
## Testing Guidelines
Unit tests live under `tests/` and are executed with pytest (`poetry run pytest`). The `tests/` package mirrors the `src/blossom/` module layout (e.g., `src/blossom/dataframe/aggregate.py` → `tests/dataframe/test_aggregate.py`). Use `test_<module>.py` as the primary filename for a module. If a test depends on Ray or Spark, mark it with `@pytest.mark.ray` or `@pytest.mark.spark` (markers are defined in `pyproject.toml`).
...
---
> Source: [Azure99/BlossomData](https://github.com/Azure99/BlossomData) — distributed by [TomeVault](https://tomevault.io).
