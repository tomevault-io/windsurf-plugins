# AI instruction files for DSC

> Sourced from [PowerShell/DSC](https://github.com/PowerShell/DSC), graded against the public Tome Standard and kept consistent across every major platform by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `` in [PowerShell/DSC](https://github.com/PowerShell/DSC).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

## Bundled Skills (2)

- [DSC](https://github.com/PowerShell/DSC/tree/main/.github/skills/create-dsc-resource/SKILL.md)
- [DSC](https://github.com/PowerShell/DSC/tree/main/.github/skills/create-resource-doc/SKILL.md)

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/PowerShell/DSC](https://github.com/PowerShell/DSC)

---

Install this config instantly:
```
npx tomevault install PowerShell/DSC
```
Source: [github.com/PowerShell/DSC](https://github.com/PowerShell/DSC).

<!-- genome:a-i-p -->
