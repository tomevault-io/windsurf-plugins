---
trigger: always_on
description: Core source lives in `src/`, implemented in TypeScript with the Vue 3 Composition API and ESM. Key entry modules include `src/index.ts` for exports, `src/ECharts.ts` for the main component, and related helpers in `src/composables/`, `src/utils.ts`, `src/types.ts`, and `src/update.ts`. Global/CE entrypoints live in `src/global.ts` and `src/wc.ts`. Styles are defined in `src/style.css` and injected via `src/style.ts`. Demo assets sit in `demo/` (Vite-powered) and should be kept in sync with new fe
---
...
---
> Source: [ecomfe/vue-echarts](https://github.com/ecomfe/vue-echarts) — distributed by [TomeVault](https://tomevault.io).
