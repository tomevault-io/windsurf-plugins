---
trigger: always_on
description: Italian tax report generator for foreign investments. Modello Redditi PF.
---
...
- **RSU cost basis = Valore Normale, not US FMV at vest.** Art. 68 c. 6 TUIR: il costo fiscalmente riconosciuto è il valore che ha concorso a formare il reddito. Per RSU l'IRPEF è stata assolta sul Valore Normale (media aritmetica dei prezzi di chiusura del mese precedente al vest, art. 9 c. 4 lett. a TUIR) — quello che appare come ITA FMV sull'Annual Withholding Statement Schwab e come reddito di lavoro dipendente sulla CU. Il cost basis del Year-End Summary è il W-2 US basis (FMV del giorno di vest), sistematicamente diverso. `schwab_parse.py` sostituisce qty × ITA FMV quando `lot.date_acquired` corrisponde a un vest date (±3d). Il campo `broker_pnl` della RTLine resta il P/L originale Schwab come colonna di riconciliazione; `gain_loss_eur` è calcolato sul Valore Normale.
...
scripts/test.sh                           # 146 tests (includes e2e against reference fixtures)
...
---
> Source: [vjt/decaf](https://github.com/vjt/decaf) — distributed by [TomeVault](https://tomevault.io).
