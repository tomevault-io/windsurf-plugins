...
- `lake build` — build both packages (`EurekaLibrary` = verified library under `library/`, zero-sorry bar; `EurekaGoals` = open goals under `goals/`, sorries expected). Toolchain auto-installs via elan from `lean-toolchain`; mathlib arrives as a binary cache (`lake exe cache get` runs via the require hook — never build mathlib from source, ADR-002).
- `lake build EurekaLibrary --wfail` — the Gate A strictness bar locally.
...
---
> Source: [eureka-aii/eureka](https://github.com/eureka-aii/eureka) — distributed by [TomeVault](https://tomevault.io).
