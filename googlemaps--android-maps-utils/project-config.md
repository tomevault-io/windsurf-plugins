...
`Record coverage history` workflow appends a row per module to
`coverage/history.csv` and regenerates `coverage/COVERAGE.md`. Both files are
generated by `.github/scripts/coverage_history.py`. Never edit them by hand.

The history records unit test coverage, which is what CI measures. The CSV
carries a `suite` column so emulator coverage could be added later without
changing the format, but no workflow runs instrumented tests today.
...
---
> Source: [googlemaps/android-maps-utils](https://github.com/googlemaps/android-maps-utils) — distributed by [TomeVault](https://tomevault.io).
