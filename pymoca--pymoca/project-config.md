---
trigger: always_on
description: Pymoca: Modelica-to-CAS translator (Python). Parses `.mo` -> AST -> flattens class hierarchies -> generates CasADi/SymPy/XML output.
---
...
## English Style
- For all prose: comments, docstrings, commit messages, docs.
- Ascii-only. Use -> instead of →, - instead of – or —.
- No ; in prose.
- American English. Behavior not behaviour, initialize not initialise.

## Code Style

- Python 3.10+, black (100 char), flake8 (bugbear, comprehensions, import-order), pyright
- **Comments concise** - Only comment if you can't determine local context from code within surrounding ~80 lines. **One line is the strong default**; add lines only for a constraint the code can't show and the reader would otherwise break. Design rationale, alternatives considered, and "why not X" belong in the commit message body, not in comments. **Don't reference context outside the function, class, module, etc.**
...
- **Commit message**:
  ...
  - Body:
    - Paragraph 1 (P1) is 1-3 sentences, high-level and durable. Open with an imperative sentence stating what was changed, using spec/domain terms (not code locations or step numbers), with the spec reference at the end if applicable. Follow with *concise* statement of the problem fixed ("Previously these were lost.") or meaning of a new feature. Do not explain the mechanism in P1. The mechanism belongs in P2.
    - Paragraph 2 (P2): non-obvious design context summary including mechanism - what ties the changes together, alternatives considered, tradeoffs. Don't reiterate the code diff.
    - Additional paragraphs and/or list: If necessary to describe anything important to understanding the diffs, but not covered above. Keep paragraphs to one topic. **If not related to P1 or P2, it should be a separate commit.**
...
For pymoca user and developer documentation see:

- [**`doc/`**](doc/index.md)

---
> Source: [pymoca/pymoca](https://github.com/pymoca/pymoca) — distributed by [TomeVault](https://tomevault.io).
