---
trigger: always_on
description: 本文件为 Claude Code (claude.ai/code) 在当前仓库中工作时提供指引。
---
...
- `utils/` 模块的 `logger` 属性在运行时会通过 `task_runner.patched_loggers()` 被替换——`BroadcastLogger` 同时写入 `log.txt` 和 stdout JSON Lines。
...
---
> Source: [cnwxi/epub_tool](https://github.com/cnwxi/epub_tool) — distributed by [TomeVault](https://tomevault.io).
