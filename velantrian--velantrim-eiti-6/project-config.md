---
trigger: always_on
description: VELANTRIM EITI is a zero-dependency, fully client-side PWA (Progressive Web App) built as a single monolithic `index.html` file (~2.5 MB). There is **no build step, no package manager, no backend, and no external database**. The app uses browser-native storage (IndexedDB, localStorage) and an in-browser SQLite WASM engine (`sql-wasm.js` / `sql-wasm.wasm`).
---
...
---
> Source: [velantrian/Velantrim-Eiti-6](https://github.com/velantrian/Velantrim-Eiti-6) — distributed by [TomeVault](https://tomevault.io).
