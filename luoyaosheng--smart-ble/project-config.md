---
trigger: always_on
description: `apps/` contains platform clients: `android/` (Kotlin + Compose), `flutter/` (Dart + Riverpod), `uniapp/` (Vue/uni-app), `desktop/electron/`, `desktop/tauri/`, `desktop/macos/`, and `ios/`. Shared BLE abstractions live in `core/ble-core/` and protocol definitions in `core/protocols/`. Hardware examples are under `hardware/esp32/LightBLE/`. Product specs, flow docs, and manual test notes live in `docs/`.
---
...
---
> Source: [LuoYaoSheng/smart-ble](https://github.com/LuoYaoSheng/smart-ble) — distributed by [TomeVault](https://tomevault.io).
