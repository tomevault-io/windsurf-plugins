---
trigger: always_on
description: - `src/`：应用主体 —— `api/`（路由 + 鉴权依赖 + Pydantic schema）、`core/`（yt-dlp 封装、下载 Worker）、`db/`（SQLite 连接 + 模型）、`services/`（任务/文件/回调/通知业务逻辑）、`utils/`（日志、工具函数）。
---
...
---
> Source: [zlxlabs/youtube_download_api](https://github.com/zlxlabs/youtube_download_api) — distributed by [TomeVault](https://tomevault.io).
