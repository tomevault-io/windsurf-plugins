# jarify

> Source: [amfaro/jarify](https://github.com/amfaro/jarify) — distributed by [TomeVault](https://tomevault.io)

Bespoke SQL linter and formatter for DuckDB, powered by sqlglot

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [amfaro/jarify](https://github.com/amfaro/jarify).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [amfaro/jarify](https://github.com/amfaro/jarify) — a repo with 0+ stars on GitHub.

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-s -->
