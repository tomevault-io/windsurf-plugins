---
trigger: always_on
description: JXPhotoBrowser — 轻量级 iOS 图片/视频浏览器。单一内核 + 协议扩展点,零数据模型依赖。
---
...
- 理解实现、追踪调用链或评估改动影响时,优先直接调用 `codegraph_explore`;查询中明确写出文件、符号或起止流程。
- 将返回的源码、调用路径和影响范围视为已读,不要再用 `grep`/逐文件读取重复验证;若提示索引滞后,直接读取所指文件。
- 索引会自动同步,无需手动执行 `codegraph sync`;工具不可用或项目未建立索引时再回退到 `rg` 和文件读取。
...
---
> Source: [JiongXing/PhotoBrowser](https://github.com/JiongXing/PhotoBrowser) — distributed by [TomeVault](https://tomevault.io).
