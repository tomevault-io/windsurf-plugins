---
trigger: always_on
description: Core runtime files live at the repo root: [`docker-compose.yml`](/root/puppy-stardew-server/docker-compose.yml), [`.env.example`](/root/puppy-stardew-server/.env.example), and the one-click installers [`quick-start.sh`](/root/puppy-stardew-server/quick-start.sh) and [`quick-start-zh.sh`](/root/puppy-stardew-server/quick-start-zh.sh).
---
