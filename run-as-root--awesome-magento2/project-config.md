...
- Discovery pipeline: `.github/workflows/discover.yml` (Mondays 06:00 UTC) scans GitHub for Magento-related repos via `GithubSearchClient` (topic searches + known-org listings), filters via `CandidateFilter` (quality gates + dedup against `data/**/*.yml` and `state/candidates.log.json`), guesses a subcategory via `CategoryGuesser`, and upserts a singleton "Magento 2 Discovery Candidates" issue via `IssueUpserter`. Checking a box on that issue fires `accept-candidate.yml` (on `issues: edited`), which appends the entry to the suggested `data/extensions/<subcategory>.yml` (or `extensions/_triage.yml` on low confidence), updates `state/candidates.log.json`, and opens a PR on a `candidates/<timestamp>` branch. Everything authenticates via the built-in `GITHUB_TOKEN`; no new secrets.
...
---
> Source: [run-as-root/awesome-magento2](https://github.com/run-as-root/awesome-magento2) — distributed by [TomeVault](https://tomevault.io).
