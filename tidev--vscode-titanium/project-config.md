---
trigger: always_on
description: Core extension code lives in `src/`. Use `src/extension.ts` as the entry point, with feature areas split into `src/commands`, `src/debugger`, `src/explorer`, `src/providers`, and `src/tasks`. Shared types and utilities live in `src/types`, `src/common`, and `src/utils.ts`.
---
...
---
> Source: [tidev/vscode-titanium](https://github.com/tidev/vscode-titanium) — distributed by [TomeVault](https://tomevault.io).
