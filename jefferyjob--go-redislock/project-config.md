---
trigger: always_on
description: 核心锁实现位于仓库根目录，例如 `lock.go`、`lock_reentrant.go`、`lock_read.go`、`lock_write.go`。运行时依赖的 Redis Lua 脚本存放在 `lua/`。适配器位于 `adapter/` 下，并按客户端版本拆分为独立子模块，例如 `adapter/go-redis/V9`、`adapter/go-zero/V1`，每个子模块都有自己的 `go.mod` 和测试。集成风格测试位于 `tests/`，示例代码位于 `examples/`，补充说明文档位于 `docs/`。
---
...
---
> Source: [jefferyjob/go-redislock](https://github.com/jefferyjob/go-redislock) — distributed by [TomeVault](https://tomevault.io).
