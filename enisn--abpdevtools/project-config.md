---
trigger: always_on
description: - CI in `.github/workflows/dotnet.yml` only runs `dotnet restore` and `dotnet build --no-restore`; the test step is commented out. For code changes, run `dotnet test tests/AbpDevTools.Tests/AbpDevTools.Tests.csproj` locally.
---
...
---
> Source: [enisn/AbpDevTools](https://github.com/enisn/AbpDevTools) — distributed by [TomeVault](https://tomevault.io).
