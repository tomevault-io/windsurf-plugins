...
Prose in `.claude/agents/*.md` that tells you to set `subagent_type` to Claude Code agent names (`orchestrate`, `resolve-pr`, `dependency-review`, `learner`, …) describes the **Claude Code** host. Those strings **fail validation** in Cursor if passed as Task types.
...
|--------|----------------|
| Issue → branch → Draft PR → implement → `make ci-fast` → handoff | `~/.agents/skills/orchestrate/SKILL.md` |
| Green CI, GraphQL `reviewThreads`, `sleep 600` | `~/.agents/skills/resolve-pr/SKILL.md` |
| Dependabot / deps / workflow risk, then PR loop | `~/.agents/skills/dependency-review/SKILL.md` |
| Post-merge learnings (optional) | `~/.agents/skills/learner/SKILL.md` |
...
---
> Source: [agorokh/ac-copilot-trainer](https://github.com/agorokh/ac-copilot-trainer) — distributed by [TomeVault](https://tomevault.io).
