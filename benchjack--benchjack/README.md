# benchjack

> Source: [benchjack/benchjack](https://github.com/benchjack/benchjack) — distributed by [TomeVault](https://tomevault.io)

AI agent benchmark hackability scanner — find evaluation vulnerabilities before they undermine your results   

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [benchjack/benchjack](https://github.com/benchjack/benchjack).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [benchjack/benchjack](https://github.com/benchjack/benchjack) — a repo with 24+ stars on GitHub.

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-s -->
