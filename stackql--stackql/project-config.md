---
trigger: always_on
description: The .NET member of the StackQL embedded-MCP family: `StackQL.Mcp` (net8.0/net9.0) acquires the `stackql` binary (explicit path, `STACKQL_MCP_BIN`, cache, local bundle, vendored `StackqlVendorBundle` resource for single-file publish, or a pin-verified download), spawns it over stdio and returns a connected `ModelContextProtocol.Core` client; `StackQL.Mcp.AgentFramework` bridges the tools into Microsoft Agent Framework / `Microsoft.Extensions.AI`. Both publish at the same version. `Directory.Build
---
