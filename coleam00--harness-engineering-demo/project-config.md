---
trigger: always_on
description: Schedulr is a B2B meeting-scheduling SaaS for sales teams. Stack: **Next.js 15 App Router + TypeScript** (frontend, `app/frontend/`) and **FastAPI + SQLAlchemy 2.0 + Alembic + Python 3.12** (backend, `app/backend/`). Postgres on host port **5433** (container 5432) via `app/docker-compose.yml`.
---
