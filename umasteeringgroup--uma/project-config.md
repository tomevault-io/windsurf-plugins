---
trigger: always_on
description: - Avoid kludgy fixes; prefer root-cause fixes based on observed event flow and resource readiness.
---
...
- For UMA editor windows with large image/preview areas and tool panels below, prefer an explicit user-resizable preview height: keep the preview/image area outside the tools scroll view, add a drag bar/splitter to resize it, default image previews to about 512px high when appropriate, and put only the lower tool panels in an expanding scroll view. Avoid automatic remaining-space measurement/clamping for these preview areas unless the user specifically asks for automatic sizing.
...
---
> Source: [umasteeringgroup/UMA](https://github.com/umasteeringgroup/UMA) — distributed by [TomeVault](https://tomevault.io).
