---
trigger: always_on
description: This is a multi-package monorepo (root, `client/`, `server/`, `scripts/`). Each has its own `node_modules` and `package.json`. The root `npm install` runs `npm install-clean` in all sub-packages via `postinstall`.
---
...
---
> Source: [microsoft/vscode-powerquery](https://github.com/microsoft/vscode-powerquery) — distributed by [TomeVault](https://tomevault.io).
