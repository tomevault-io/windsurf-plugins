---
trigger: always_on
description: This document gives working context for AI agents editing this repository.
---
...
- **Bun path**: Bun is installed to `~/.bun/bin/bun`. The update script ensures `$HOME/.bun/bin` is on `PATH`.
- **Env file**: The dev server reads `.env.local` (created from `.env.example`). No secrets are required — all upstream APIs are public.
- **No database or Docker needed**: The app has no local database. All data comes from remote public Farcaster APIs (Snapchain, Hypersnap, Farcaster API, Optimism RPC). Redis is optional and the app degrades gracefully without it.
- **Dev server**: `bun run dev` starts a Vite dev server on port 3000 with HMR. It connects to live upstream services on startup (gRPC for event stream, REST for API calls).
- **Lint**: `bun run lint` runs ESLint (flat config in `eslint.config.mjs`). No TypeScript type-checking command is configured separately.
- **Build**: `bun run build` produces Nitro output in `.output/`. Production server runs via `node .output/server/index.mjs`.
- **No test framework**: The project has no automated test suite. Validate changes via lint + build + manual testing of the dev server.

---
> Source: [dylsteck/casterscan](https://github.com/dylsteck/casterscan) — distributed by [TomeVault](https://tomevault.io).
