...
For **when tests are required** (bugs, new public APIs, refactors), see the test policy table in `agent-clean-code.mdc`.
...
- **Coverage and xdist:** Do NOT add `--cov=asap` or `--cov-report=*` to pytest `addopts` in `pyproject.toml`. Running coverage with pytest-xdist can cause INTERNALERROR. Run coverage only in a dedicated job without `-n auto` (e.g. `uv run pytest --tb=short --cov=asap --cov-report=xml --cov-fail-under=85`). The fail-under value is the minimum project coverage threshold; do not lower it without team agreement.
