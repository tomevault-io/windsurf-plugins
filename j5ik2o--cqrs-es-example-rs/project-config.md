---
trigger: always_on
description: - ルートは Cargo ワークスペースで、`applications/` が実行可能バイナリを持つ；`write-api-server` は GraphQL Mutation、`read-api-server` は Query、`read-model-updater` はリードモデル更新ロジックを提供する。
---
...
---
> Source: [j5ik2o/cqrs-es-example-rs](https://github.com/j5ik2o/cqrs-es-example-rs) — distributed by [TomeVault](https://tomevault.io).
