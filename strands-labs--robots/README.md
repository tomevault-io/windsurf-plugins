# robots

> Source: [strands-labs/robots](https://github.com/strands-labs/robots). Graded, signed and kept current as the models change by [TomeVault](https://tomevault.io)

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `AGENTS.md` in [strands-labs/robots](https://github.com/strands-labs/robots).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [strands-labs/robots](https://github.com/strands-labs/robots) — a repo with 104+ stars on GitHub.

---

Install this config instantly:
```
npx tomevault install strands-labs/robots
```
Source: [github.com/strands-labs/robots](https://github.com/strands-labs/robots).

<!-- genome:d-i-s -->
