...
Detailed prompt templates live in `<SKILL_DIR>/commands/`. In Letta Code, install those markdown files into a supported custom-command directory such as `~/.letta/commands/visual-explainer/` and invoke the Letta-safe command names like `/ve-diff-review`, `/ve-generate-web-diagram`, and `/ve-share`. You can also load the skill directly with the `Skill` tool as `$visual-explainer` when needed.
...
For prose accents, see "Prose Page Elements" in `<SKILL_DIR>/references/css-patterns.md`. For everything else, use the standard freeform approach with aesthetic directions below.
...
- For text-heavy architecture overviews (card content matters more than topology): read `<SKILL_DIR>/templates/architecture.html`
- For flowcharts, sequence diagrams, ER, state machines, mind maps, class diagrams, C4: read `<SKILL_DIR>/templates/mermaid-flowchart.html`

<!-- Content truncated to meet Windsurf 6KB limit -->

---
> Source: [ameno-/letta-explainer](https://github.com/ameno-/letta-explainer) — distributed by [TomeVault](https://tomevault.io).
