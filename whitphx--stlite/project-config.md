---
trigger: always_on
description: - `packages/` hosts the Yarn workspaces: `browser`, `sharing`, and `desktop` are user apps; `react` is a React library that the apps depend on; `kernel` is the Pyodide core that `react` depends on; `sharing-editor` is the editor client for `sharing`, and the rest (`common`, `sharing-common`, `tooling`) are support utilities.
---
