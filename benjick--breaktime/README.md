# breaktime

> Source: [benjick/breaktime](https://github.com/benjick/breaktime) — distributed by [TomeVault](https://tomevault.io)

A macOS menu bar app that reminds you to take breaks to prevent RSI

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [benjick/breaktime](https://github.com/benjick/breaktime).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [benjick/breaktime](https://github.com/benjick/breaktime) — a repo with 0+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
