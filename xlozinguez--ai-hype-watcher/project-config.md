---
trigger: always_on
description: Curated AI-assisted development learning resource — from fundamentals to agentic orchestration. Includes critical perspective on AI industry economics and hype cycles.
---
...
STATE.md          → Living summary of validated AI engineering theses (generated by /research-curriculum)
```
...
| `/state-of-field` | Get current validated understanding of AI engineering state for cross-referencing |
...
```
/research-curriculum all                # Full curriculum audit (runs weekly on Sundays)
/research-curriculum 04 core-concepts   # Validate one section
/state-of-field                         # Get current thesis statuses
/state-of-field security                # Filter by topic
```

Research reports are stored in `research/` and the living summary in `STATE.md`.
A weekly scheduled run validates all modules every Sunday and creates a PR with findings.

#### Weekly schedule setup

The weekly research validation is configured as a remote scheduled agent (trigger) via Claude Code's `/schedule` skill. To create or update it:
...
Configuration:
- **Name**: `weekly-research-validation`
- **Cron**: `0 12 * * 0` (every Sunday at 8am ET / 12:00 UTC)
- **Model**: `claude-sonnet-4-6`
- **Repo**: `https://github.com/xlozinguez/ai-hype-watcher`
- **Environment**: `claude-code-default`
- **Prompt**:
  > Run `/research-curriculum all` to validate all 6 curriculum modules against sources and external evidence. After the research run completes, regenerate STATE.md from the research reports. Then activate the bot identity with `bash scripts/bot-auth.sh activate`, create a branch `research/weekly-YYYY-MM-DD`, commit all changes, push, and open a PR titled "research: weekly validation YYYY-MM-DD" summarizing findings. Use the bot token for gh commands: `GH_TOKEN=$(jq -r .token /tmp/.bot-token-cache) gh pr create ...`

To manage the trigger after creation, visit: https://claude.ai/code/scheduled
...
- When producing content (briefings, synthesis, curriculum, podcasts), cross-reference STATE.md for current thesis statuses

---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/xlozinguez)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/xlozinguez)
