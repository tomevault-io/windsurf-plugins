...
Follow the Stylua config: 2-space indentation, 120-column cap, no call parentheses for simple calls. Modules are addressed as `require('numb.<submodule>')`; keep filenames lowercase with underscores only when mirroring Neovim option names. Prefer descriptive local names (`peek_line`, `cursor_state`) over single letters, and return tables that expose only the documented API.

## Testing Guidelines
Run `scripts/check.sh` (or directly invoke `nvim --headless -u tests/init.lua -i NONE -n +"lua require('tests.run').run()" +qall`) to execute the automated headless regression tests covering option restoration, sequential jumps, out-of-bounds clamps, and both absolute (`:5`) and relative (`:+2`) motions. Extend `tests/run.lua` whenever you fix a bug or add a new option. For exploratory work, still verify inside Neovim using `:lua require('numb').setup{centered_peeking=false}` and `:{number}` jumps. Document any manual scenarios you covered in the PR description. CI (`.github/workflows/ci.yml`) runs the same script on every push to and pull request targeting `master`.
...
---
> Source: [nacro90/numb.nvim](https://github.com/nacro90/numb.nvim) — distributed by [TomeVault](https://tomevault.io).
