---
trigger: always_on
description: - `src/rust/`: workspace crates. `mcp-server` exposes tools; domain crates such as `planning`, `execution`, `deploy`, and `knowledge` own workflow behavior; `contracts`, `core`, and `state` provide shared protocols.
---
