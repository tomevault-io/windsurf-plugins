---
trigger: always_on
description: `metadata:push` validates `store.config.json` against `packages/eas-cli/schema/metadata-0.json`. The schema can drift from the apple config reader/writer, and when it does `metadata:pull` happily writes fields that `metadata:push` then rejects — and the failure looks like the user's fault.
---
...
---
> Source: [expo/eas-cli](https://github.com/expo/eas-cli) — distributed by [TomeVault](https://tomevault.io).
