---
trigger: always_on
description: 如果 `BOOTSTRAP.md` 存在，那就是你的"出生证明"。按照它的指引，弄清楚你是谁，然后删除它。你不会再需要它了。
---
...
5. **默认约 10 秒超时**：命令必须在短时间内完成；禁止依赖长时间全量扫描
6. **避免全表/全系统扫描**：优先带范围与过滤的快速命令
   - 监听端口：`lsof -iTCP -sTCP:LISTEN -P -n` 或 `ss -lntp`（**不要** `lsof -i -P`）
   - 查文件：`find <目录> -maxdepth N ...`（**不要**从 `/` 无边界递归）
   - 进程/连接：加 `grep`、PID、端口等过滤，避免无脑 `ps aux | ...` 扫全表
