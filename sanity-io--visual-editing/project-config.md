...
- **Extracted schemas are committed:** each demo app has its extracted Sanity schema checked into git as `apps/*/schema.json`, used by `sanity typegen generate`. After changing `@repo/sanity-schema`, run `pnpm typegen` (root script) to re-extract the schemas and regenerate the typegen output, then commit the results. The `sanity schema extract` step prints harmless warnings about workspaces declaring different `auth` configs; these are non-fatal.

---
> Source: [sanity-io/visual-editing](https://github.com/sanity-io/visual-editing) — distributed by [TomeVault](https://tomevault.io).
