# LangChain.Providers

> Source: [tryAGI/LangChain.Providers](https://github.com/tryAGI/LangChain.Providers) — distributed by [TomeVault](https://tomevault.io)

Part of the LangChain.NET project. Has separate abstractions, does not contain dependencies on the main project and can be used independently

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [tryAGI/LangChain.Providers](https://github.com/tryAGI/LangChain.Providers).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [tryAGI/LangChain.Providers](https://github.com/tryAGI/LangChain.Providers) — a repo with 23+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
