# talos

> Source: [ob-labs/talos](https://github.com/ob-labs/talos) — distributed by [TomeVault](https://tomevault.io)

Talos is a Ralph CLI，It enables you to execute multiple Ralph Loop tasks in parallel across multiple repositories.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [ob-labs/talos](https://github.com/ob-labs/talos).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/ob-labs/talos](https://github.com/ob-labs/talos)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-q -->
