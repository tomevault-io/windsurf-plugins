---
trigger: always_on
description: This file provides guidance to Claude Code when working with the Qubx quantitative trading framework.
---
...
uv run qubx init --name my_strategy --symbols BTCUSDT,ETHUSDT --exchange BINANCE.UM

# Create strategy with specific template
uv run qubx init --name my_strategy --template project
...
uv run qubx simulate config.yml

# Validate strategy configuration
uv run qubx validate config.yml

# Validate without checking strategy imports
uv run qubx validate config.yml --no-check-imports

# List strategies in directory
uv run qubx ls

# Package strategy into release
uv run qubx release --config config.yml

# Deploy strategy from zip file
uv run qubx deploy --file strategy.zip
...
Use `uv run qubx init` to create strategies from templates:
...
---
> Source: [xLydianSoftware/Qubx](https://github.com/xLydianSoftware/Qubx) — distributed by [TomeVault](https://tomevault.io).
