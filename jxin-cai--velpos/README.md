# velpos

> Source: [Jxin-Cai/velpos](https://github.com/Jxin-Cai/velpos) — distributed by [TomeVault](https://tomevault.io)

Rapidly build a multi-Agent management dashboard based on Claude Code, integrating daily work and remote IM collaboration. It includes Claude Code monitoring, channel configuration and message notifications, with a Codex GUI optimized for non-technical users.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [Jxin-Cai/velpos](https://github.com/Jxin-Cai/velpos).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/Jxin-Cai/velpos](https://github.com/Jxin-Cai/velpos)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-q -->
