---
trigger: always_on
description: - 只改动必要的部分，优先复用现有成熟代码，避免重复造轮子。
---
...
- 如目前无对应语言的测试与验证要求，应当在项目根目录的 `GEMINI.md` 中结合项目实际情况补齐要求。
...
- 对于复杂任务（例如涉及多个模块或跨领域变更），必须先制定分阶段实施计划并创建任务文档，然后按阶段使用 Todo Tool 规划并推进执行。每完成一个阶段，必须更新文档中的TODO状态。
...
---
> Source: [Swfdong/reliable-coding-agent](https://github.com/Swfdong/reliable-coding-agent) — distributed by [TomeVault](https://tomevault.io).
