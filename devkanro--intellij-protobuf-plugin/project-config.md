---
trigger: always_on
description: This project uses a structured Copilot workflow with custom skills. Read this before doing anything.
---
...
1. **Brainstorm** — Design the solution in `.github/brainstorm.md` before writing code. The brainstorm skill manages a state machine (Planning → Questioning → Ready → Implementing → Completed) that prevents premature coding.
...
4. **Reflect** — After every ship, reflect on what happened. Learnings are captured in `.github/knowledge.md` and feed back into future work.
...
| File | Purpose |
|---|---|
| `.github/brainstorm.md` | Active design discussion (one per branch) |
| `.github/knowledge.md` | Accumulated learnings and patterns |
| `.github/docmap.md` | Documentation structure and code-to-doc mapping |
| `.github/copilot-instructions.md` | This file — workflow overview |
| `.github/skills/` | Custom skill definitions |

## Knowledge Base

`.github/knowledge.md` is a living document that grows with each reflection. It contains:
...
| Skill | Purpose |
|---|---|
| `brainstorm` | Design discussions via `.github/brainstorm.md` state machine |
...
1. **No code changes during design** — Until brainstorm reaches `Implementing`, only `.github/brainstorm.md` may be modified
...
5. **Keep knowledge current** — Update `.github/knowledge.md` after each reflection, curate patterns when they recur

---
> Source: [devkanro/intellij-protobuf-plugin](https://github.com/devkanro/intellij-protobuf-plugin) — distributed by [TomeVault](https://tomevault.io).
