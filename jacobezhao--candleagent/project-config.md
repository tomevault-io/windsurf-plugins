---
trigger: always_on
description: CandleAgent has a FastAPI backend in `backend/app/` and a Vite/React frontend
---
...
CandleAgent has a FastAPI backend in `backend/app/` and a Vite/React frontend
...
`G:/CandleMind/CandleMind_data`, never in a repository `data/` tree. This legacy
physical path is retained for compatibility with existing runtime state and artifacts.
...
---
> Source: [JacobeZhao/CandleAgent](https://github.com/JacobeZhao/CandleAgent) — distributed by [TomeVault](https://tomevault.io).
