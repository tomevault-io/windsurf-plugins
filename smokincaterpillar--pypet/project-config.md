---
trigger: always_on
description: pypet (Python Parameter Exploration Toolkit) is a library for managing
---
...
- These prefixes avoid name collisions with natural naming tree access

### Mixin Hierarchy

- `HasSlots` (`slots.py`) - metaclass-based `__slots__` management
- `HasLogger` (`pypetlogging.py`) - logging mixin (inherits HasSlots)
- `WithAnnotations` (`annotations.py`) - annotation support

### Storage

HDF5 storage via PyTables with multiple serialization protocols.
The `storageservice.py` module handles loading/storing trajectory
trees to HDF5 files.
...
- `QUEUE` / `PIPE` / `LOCK` / `LOCAL` - standard multiprocessing
...
- **Framework**: unittest
- **Test discovery**: `LambdaTestDiscoverer`
  (`pypet/tests/testutils/ioutils.py`) - tag-based filtering
  using lambda predicates
- **Tags**: `unittest`, `integration`, `multiproc`, `hdf5`,
  `hdf5_settings`, `merge`, `links`
- **Base class**: `TrajectoryComparator(unittest.TestCase)`
  in `pypet/tests/testutils/data.py`
- **CI**: GitHub Actions (`tests.yml`) - Python 3.8 (multiproc)
  and 3.10 (singlecore), conda-based

## Modernization Notes

Areas needing updates for modern Python:

- **Build system**: `setup.py` with distutils fallback ->
  `pyproject.toml`
- **Python version**: `python_requires='>=3.6'` but 3.6/3.7
  are EOL; classifiers only go to 3.8
- **Python 2 remnants**: `FileNotFoundError = IOError` compat
  stub in `pypetlogging.py:5-9`
- **CI**: Uses `setup-miniconda@v2`, `actions/checkout@v3`
  (outdated)
- **No type annotations** anywhere in the codebase
- **No pre-commit hooks**, no modern formatter (black/ruff),
  no linter config
- **No pyproject.toml** - all config in `setup.py`
- **`object` base classes**: e.g.,
  `HasSlots(object, metaclass=...)` - unnecessary in Python 3
- **m2r** for README conversion - deprecated, use `m2r2`
  or just set `long_description_content_type`

---
> Source: [SmokinCaterpillar/pypet](https://github.com/SmokinCaterpillar/pypet) — distributed by [TomeVault](https://tomevault.io).
