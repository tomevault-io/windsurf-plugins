# auditagent-scoring-algo

> Source: [NethermindEth/auditagent-scoring-algo](https://github.com/NethermindEth/auditagent-scoring-algo) — distributed by [TomeVault](https://tomevault.io)

AuditAgent scoring algorithm to evaluate the results of AI security scans versus a source of truth.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [NethermindEth/auditagent-scoring-algo](https://github.com/NethermindEth/auditagent-scoring-algo).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [NethermindEth/auditagent-scoring-algo](https://github.com/NethermindEth/auditagent-scoring-algo) — a repo with 18+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
