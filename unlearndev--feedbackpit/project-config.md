...
- Never set foreign key IDs manually (e.g. `'user_id' => $user->id`). Always use Eloquent relationships (`associate()`, or create through the relationship) to set foreign keys.
- Don't create temporary variables just to pass data into API Resources or Inertia responses. Pass the query/expression directly inline.
...
---
> Source: [unlearndev/feedbackpit](https://github.com/unlearndev/feedbackpit) — distributed by [TomeVault](https://tomevault.io).
