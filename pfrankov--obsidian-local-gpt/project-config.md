---
trigger: always_on
description: - `src/`: Core plugin code. Entry is `src/main.ts`; RAG logic in `src/rag.ts`; settings UI in `src/LocalGPTSettingTab.ts`; helpers in `utils.ts`, `indexedDB.ts`, `request-handler.ts`, `i18n/`.
---
...
---
> Source: [pfrankov/obsidian-local-gpt](https://github.com/pfrankov/obsidian-local-gpt) — distributed by [TomeVault](https://tomevault.io).
