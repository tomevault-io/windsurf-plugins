---
trigger: always_on
description: This guide targets DM contributions in the TiFlow monorepo. Core DM code lives in `dm/`, shared libraries commonly touched by DM live in `pkg/`, and DM binaries are launched from `cmd/dm-master`, `cmd/dm-worker`, `cmd/dm-ctl`, and `cmd/dm-syncer`. Integration and compatibility cases live in `dm/tests/`; reusable helpers are under `dm/tests/_utils/`. The DM web UI is in `dm/ui/`. `sync_diff_inspector/` is also relevant because DM integration tests depend on it.
---
...
---
> Source: [pingcap/tiflow](https://github.com/pingcap/tiflow) — distributed by [TomeVault](https://tomevault.io).
