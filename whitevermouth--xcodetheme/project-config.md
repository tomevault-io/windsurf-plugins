---
trigger: always_on
description: This repository is a JetBrains theme plugin built with Gradle. Production code lives in `src/main/java/com/vermouthx/xcodetheme/` and is organized by concern: `activities/`, `listeners/`, `notifications/`, `settings/`, and `enums/`. Theme definitions and plugin metadata live in `src/main/resources/`, including `*.theme.json`, editor schemes `*.xml`, and `META-INF/plugin.xml`. Marketing assets such as screenshots and logos are stored in `assets/`.
---
