---
trigger: always_on
description: Nauthilus development guidelines (project-specific)
---
...
0. Non-negotiable rules (must always be enforced)

- These rules are mandatory for every coding change. A task is incomplete if any mandatory rule is missed.
...
- External dependencies and isolation
  ...
  - Debugging rule for Lua issues: first create a Lua-focused reproducer test (preferably JSON-driven using fixtures
    from testdata when applicable) that reproduces the problem. Good tests that improve coverage may be kept in the
    repository.
  - Debugging rule for Go issues: apply the same approach and first create a focused Go test that reproduces the
    problem. Good tests that improve coverage may be kept in the repository.
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/croessner)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/croessner)
