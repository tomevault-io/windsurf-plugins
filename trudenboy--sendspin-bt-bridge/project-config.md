---
trigger: always_on
description: Dockerized Python bridge between Music Assistant's Sendspin protocol and Bluetooth speakers. Designed for headless systems (Raspberry Pi, Home Assistant addon, Proxmox LXC, OpenWrt LXC).
---
...
Single `VERSION` file triggers the unified `release.yml` pipeline: lint → pytest → config.py update + tag → Docker build (amd64/arm64) → HA addon sync → GitHub Release (stable) → armv7 (stable). Regular pushes/PRs use `ci.yml` (lint + test only). Images published to `ghcr.io/trudenboy/sendspin-bt-bridge`.

---
> Source: [trudenboy/sendspin-bt-bridge](https://github.com/trudenboy/sendspin-bt-bridge) — distributed by [TomeVault](https://tomevault.io).
