---
trigger: always_on
description: Local dir is `repo-audit-action/`; the **published name is `wei18/upkeep`** — the mismatch is intentional. Full reference: `docs/en/design.md`.
---
...
- `dist/` is git-ignored, never committed; `npm run build` (tsc) is a type-check gate only — CI and the composite actions run `src/*.ts` directly via tsx.
...
---
> Source: [wei18/Upkeep](https://github.com/wei18/Upkeep) — distributed by [TomeVault](https://tomevault.io).
