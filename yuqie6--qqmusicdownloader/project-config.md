---
trigger: always_on
description: 核心 TUI 实现在 `src/qqmusicdownloader/`，按分层结构划分：`ui/` 负责 Textual 布局与交互，`services/` 编排下载流程，`domain/` 保存实体与配置，`infrastructure/` 对接 QQ 音乐 API、加密脚本与文件系统。静态资源放在 `resources/`。终端示例与额外资料位于 `docs/`。回归测试集中在 `tests/`，务必与业务模块一一对应。`music-downloader-site/` 为站点素材，修改前确认不会破坏 TUI 主流程。
---
...
---
> Source: [yuqie6/qqmusicdownloader](https://github.com/yuqie6/qqmusicdownloader) — distributed by [TomeVault](https://tomevault.io).
