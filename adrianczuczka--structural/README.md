# structural

> Source: [adrianczuczka/structural](https://github.com/adrianczuczka/structural) — distributed by [TomeVault](https://tomevault.io)

A lightweight Gradle plugin for enforcing package dependency rules in Android & Kotlin projects.

## Windsurf Config

The `project-config.md` file in this directory is the project config converted for Windsurf.
Original source: `CLAUDE.md` in [adrianczuczka/structural](https://github.com/adrianczuczka/structural).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [adrianczuczka/structural](https://github.com/adrianczuczka/structural) — a repo with 18+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
