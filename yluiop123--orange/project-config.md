---
trigger: always_on
description: - **底层组件**: 要创建的组件中能复用shadcn组件的部分，直接复用（比如单选和复选组件），复用不了的则用Radix UI + Tailwind CSS实现。
---
...
2. **全面示例页面**: 位于 `@/pages/component/{dir}/{name}.tsx`（或类似路径 `dir`路径我到时候会指定，`name`你根据规范生成）。
...
---
> Source: [yluiop123/orange](https://github.com/yluiop123/orange) — distributed by [TomeVault](https://tomevault.io).
