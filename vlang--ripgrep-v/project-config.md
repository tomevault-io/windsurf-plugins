---
trigger: always_on
description: These rules apply to all Rust-to-V translated code in this repository.
---
...
4. A handwritten Rust `impl Clone` maps to an explicit V `clone()` with the same logic.

5. Only keep a handwritten `clone()` when the type needs custom behavior that the autogenerated `IClone` path does not express cleanly.
...
---
> Source: [vlang/ripgrep.v](https://github.com/vlang/ripgrep.v) — distributed by [TomeVault](https://tomevault.io).
