---
trigger: always_on
description: Core application code lives under `project/`. Use `project/core/` for app setup, settings, database wiring, and service endpoints such as health and metrics. Feature code belongs in `project/apps/`; the current example is `project/apps/user/` with `models.py`, `serializers.py`, and `views.py`. Static assets and templates are served from `project/static/`, `project/media/`, and `project/templates/`. Alembic migrations live in `migrations/`. Tests mirror the runtime structure under `tests/` (`test
---
...
---
> Source: [p141592/simple_fastapi](https://github.com/p141592/simple_fastapi) — distributed by [TomeVault](https://tomevault.io).
