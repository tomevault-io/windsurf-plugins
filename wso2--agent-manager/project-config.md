---
trigger: always_on
description: A **Kubernetes init container** (Docker image only — no Python package) that injects Python auto-instrumentation into application pods. It copies the instrumentation packages into a shared volume that the app pod mounts onto its `PYTHONPATH`, so the app is instrumented with zero code change.
---
