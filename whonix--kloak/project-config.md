---
trigger: always_on
description: Guidelines learned from upstream review of security hardening patches
---
...
- Always declare and initialize all variables used in a function at the
  beginning of the function, even if the initialization value will never be
  used. This reduces the risk of a refactor causing an uninitialized value to
  be used.
...
To analyze the source code with `cppcheck`, run the following command in the
root of the `kloak` project's source tree:
...
Do not enable the following checks:

- `style`. These checks are mostly aesthetic, and disagree with the Variable
  Initialization rules specified above.
- `information`. These "checks" are not checks, but rather summary information
  that is of no value when looking for vulnerabilities.
- `missingInclude`. This will print copious warnings about standard library
  headers. It will also warn about missing headers that are autogenerated from
  Wayland protocol files, and if one generates those Wayland protocol headers to
  satisfy `missingInclude, `unusedFunction` will print copious warnings about
  unused functions in those headers.

Any messages that are printed by `cppcheck` when run as specified above must
be either fixed or silenced, as appropriate.

---
> Source: [Whonix/kloak](https://github.com/Whonix/kloak) — distributed by [TomeVault](https://tomevault.io).
