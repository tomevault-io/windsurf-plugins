---
trigger: always_on
description: - When a workflow or `SCBuildTest` starts using a new packaged toolchain, runner, or sysroot from `Tools/SC-package.cpp`, update the GitHub Actions caches in the matching workflow in the same change.
---
...
- The same rule applies on Posix: before adding a separate warmup job, check whether only one lane per OS actually uses the package. If so, gating the restore to those lanes is usually simpler and faster than serializing the whole workflow behind a warmup job.
- For large immutable archives in `Tools/SC-package.cpp`, prefer `functions.keepDownloadedArchive = false` once the extracted layout is enough to reinstall or validate the package. Otherwise the workflow cache can silently store both the archive and the extracted tree.
...
---
> Source: [Pagghiu/SaneCppLibraries](https://github.com/Pagghiu/SaneCppLibraries) — distributed by [TomeVault](https://tomevault.io).
