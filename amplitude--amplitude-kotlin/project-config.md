---
trigger: always_on
description: Analytics SDK for Android. Two modules: **`analytics-core`** (pure Kotlin/JVM) and **`android`** (extends analytics-core with Android lifecycle, sessions, autocapture). Published on Maven Central as `com.amplitude:analytics-core` and `com.amplitude:analytics-android`.
---

# Amplitude Kotlin SDK

Analytics SDK for Android. Two modules: **`analytics-core`** (pure Kotlin/JVM) and **`android`** (extends analytics-core with Android lifecycle, sessions, autocapture). Published on Maven Central as `com.amplitude:analytics-core` and `com.amplitude:analytics-android`.
...
- Every change ships with unit tests (JUnit 5, Robolectric, MockK, `kotlinx-coroutines-test`, MockWebServer).
  - Core: `analytics-core/src/test/kotlin/` | Android: `android/src/test/java/`
...
- Android-specific code belongs in `:android`, not `:analytics-core`. Core stays platform-independent.
...
- Key plugin interfaces: `Plugin`, `EventPlugin`, `DestinationPlugin`, `ObservePlugin` in `analytics-core/.../platform/Plugin.kt`.
...
---
> Source: [amplitude/Amplitude-Kotlin](https://github.com/amplitude/Amplitude-Kotlin) — distributed by [TomeVault](https://tomevault.io).
