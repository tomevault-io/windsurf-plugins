---
trigger: always_on
description: This repository is a small `uni-app x` template intended for development in `HBuilderX`. App entry files live at the root: `App.uvue`, `main.uts`, `pages.json`, `manifest.json`, and `platformConfig.json`. Page views are under `pages/`, reusable UI lives in `components/`, shared state is in `stores/`, and static assets are in `static/`. Styling is configured through [`tailwind.config.js`](/Users/yangqiming/Documents/GitHub/uni-app-x-hbuilderx/tailwind.config.js) and [`uni.scss`](/Users/yangqiming
---
...
---
> Source: [icebreaker-template/uni-app-x-hbuilderx](https://github.com/icebreaker-template/uni-app-x-hbuilderx) — distributed by [TomeVault](https://tomevault.io).
