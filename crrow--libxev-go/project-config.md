...
* 你正在协助的对象是 **Ryan**。
* 假设 Ryan 是一名经验丰富的资深后端 / 数据库工程师，熟悉 Rust、Go、Python 等主流语言及其生态。
* 本项目尚未上线，无需考虑向后兼容性；可以自由重构、重命名、调整 API，无历史包袱。
* Ryan 重视“Slow is Fast”，关注点在于：推理质量、抽象与架构、长期可维护性，而不是短期速度。
* 你的核心目标：
...
* 完成代码修改后，必须主动运行合适的验证命令（至少 `cargo check` 或对应测试），修复错误直至能够运行，再进行最终输出。

### 1.11 主动检查

* 发现设计或者实现中的问题时，需要主动去修复，而不是再跟Ryan确认。
...
* 命名与格式：
  ...
  * Python：遵循 PEP 8；
  * 其他语言遵循对应社区主流风格。
* 在给出较大代码片段时，默认该代码已经过对应语言的自动格式化工具处理（如 `cargo fmt`、`gofmt`、`black` 等, 一般这种format都会有配置，需要使用justfile中编写好的fmt rule， 如just fmt）。
* 注释：
  * 仅在行为或意图不明显时添加注释；
...
完整风格指南见 [docs/rust-style.md](docs/rust-style.md)。
...
* 默认私有，按需放宽
* `pub(super)` 优于 `pub(crate)`，`pub(crate)` 优于 `pub`
* 类型别名与内部类型可见性匹配：`pub(super) type Foo = Arc<FooInner>`
...
```rust
// Good: intent is clear, temporaries are scoped
let config = {
...
let items = {
    let mut v = Vec::new();
    v.extend([1, 2, 3]);
    v // returned as immutable
};
```

适用：3-15 行构造逻辑。超过则提取为函数。

#### 4.2.2 Newtype Pattern
...
* 使用 `?` 传播错误；避免 `.unwrap()`，除非在测试或逻辑上确保安全
* 库代码用 `thiserror`，应用代码用 `anyhow`
* `Option` 表示缺失，`Result` 表示失败；不要混淆
* 尽早返回错误，保持 happy path 不缩进：

```rust
// Good: early return
let Some(value) = maybe_value else { return Ok(()) };
...
// Acceptable: when loop body is complex or has side effects
for item in items {
    if item.valid {
        process(&item)?;
    }
...
* 最小化 `Arc<Mutex<T>>` 使用范围；优先消息传递或无锁结构
* 性能敏感时用 `parking_lot` 替代 `std::sync`
* 禁止在 `.await` 点持有锁
* 异步代码中优先使用 `tokio::sync` 原语

#### 4.2.7 Worker Pattern

后台 worker 使用 **直接启动模式**：`start() -> WorkerHandle`，而非 `new()` + `start()` 两步式。

详见 [docs/worker-pattern.md](docs/worker-pattern.md)。
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/crrow)
> This is a context snippet only. You'll also want the standalone SKILL.md file — [download at TomeVault](https://tomevault.io/claim/crrow)
