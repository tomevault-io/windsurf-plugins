...
After making changes to any plugin, always run these three steps as the final pass:

1. **Push** — `git push` to publish changes to GitHub
2. **Update** — `just update` to pull the latest marketplace and update all installed plugins
3. **Status** — `just status` to verify installed versions match repo versions (all green)
...
4. **PUSH TO REMOTE** - This is MANDATORY:
   ...
   git push
   just update
   just status  # MUST show all green
   ```
...
---
> Source: [florianbuetow/claude-code](https://github.com/florianbuetow/claude-code) — distributed by [TomeVault](https://tomevault.io).
