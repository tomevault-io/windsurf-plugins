---
trigger: always_on
description: Kubernetes mutating admission webhook that manages MAC address allocation from a cluster-level pool for Pods and KubeVirt VirtualMachines on secondary networks (Multus/NetworkAttachmentDefinition).
---
...
- Handles the webhook cache side effect by recording allocation state on the VM with a transaction-state (TS) annotation
...
No custom CRDs. Works with standard Kubernetes and KubeVirt resources:
- **Pods** are MAC allocation targets
- **VirtualMachines** are MAC allocation targets
- **VirtualMachineInstances** are monitored for collision detection
...
---
> Source: [k8snetworkplumbingwg/kubemacpool](https://github.com/k8snetworkplumbingwg/kubemacpool) — distributed by [TomeVault](https://tomevault.io).
