---
trigger: always_on
description: This repository contains the TextMate grammar for C# with outputs for VSCode, Atom, and TextMate. The grammar defines syntax highlighting rules for C# code.
---
...
- Run `npm run prepublish` to build and test the grammar
  - This runs the npm script pipeline in `package.json` which:
...
3. Run `npm run prepublish` to build and test the grammar
...
- After making changes, run `npm run prepublish` to run the full test suite
...
---
> Source: [dotnet/csharp-tmLanguage](https://github.com/dotnet/csharp-tmLanguage) — distributed by [TomeVault](https://tomevault.io).
